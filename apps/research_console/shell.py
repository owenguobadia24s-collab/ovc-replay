...
def render_overview(bundle: Mapping[str, Any]) -> None:
    ...
    with metrics[0]:
        render_metric_card("Fixture objects", str(len(bundle.get("objects", []))), "No live projections consumed")
    with metrics[1]:
        render_metric_card("Health domains", str(len(bundle.get("health", []))), "No-signal is never PASS")
    with metrics[2]:
        render_metric_card("Release fixtures", str(len(bundle.get("releases", []))), "Inspection only")
    with metrics[3]:
        render_metric_card("Queue attention", "2 due · 1 censored", "Fixture consequence model")
    ...
    with left:
        ...
        if releases:
            ...
            if st.button("Inspect development release", key="overview::release"):
                _select_object("RELEASE.DEV.2025")
        else:
    ...
    with middle:
        ...
        if gates:
            ...
            if st.button("Inspect RC-A1", key="overview::gate"):
                _select_object("GATE.RC_A1")
        else:
    ...
    with right:
        ...
        if queue_items:
            ...
            if st.button("Inspect due realization", key="overview::queue"):
                _select_object("QUEUE.REALIZATION.001")
        else:
...
def render_research(bundle: Mapping[str, Any]) -> None:
    ...
    with replay_col:
        ...
        if replay:
            ...
            st.caption("Green cutoff marker is represented by the declared fixture horizon; post-cutoff values are absent in PROSPECTIVE mode.")
        else:
    ...
    with queue_col:
        ...
        for item in queue_items:
            st.markdown(f"{status_html(str(item['status']))} **{escape(str(item['label']))}**", unsafe_allow_html=True)
            st.caption(str(item["summary"]))
            if st.button("Inspect", key=f"research::queue::{item['object_id']}", use_container_width=True):
                _select_object(str(item["object_id"]))
    ...
    if evidence:
        ...
        for index, item in enumerate(evidence):
            with columns[index % 4]:
                if render_evidence_card(item, button_key=f"evidence::{item['object_id']}"):
                    _select_object(str(item["object_id"]))
    else:
...
def render_system(bundle: Mapping[str, Any]) -> None:
    ...
    if section == "HEALTH":
        ...
        if objects:
            ...
            if st.button("Open selected object", key="system::lineage::open"):
                _select_object(str(selected))
        else:
        ...
        if releases:
            ...
            render_empty_state("NOT_EVALUATED", "No fixture releases are present.", "No release claim is made.", "Select another fixture condition.")
    elif section == "QA_GATES":
        gates = list(bundle.get("gates", []))
        if gates:
            st.dataframe(gates, use_container_width=True, hide_index=True)
        else:
            render_empty_state("NOT_EVALUATED", "No gate fixtures are present.", "No assurance claim is made.", "Restore the fixture gate source.")
    elif section == "AUDIT":
...
def render_activity_stream(bundle: Mapping[str, Any]) -> None:
    ...
    with st.expander("Activity stream · persistent, source-derived and read-only", expanded=bool(st.session_state.activity_open)):
        st.session_state.activity_open = True
        render_activity_rows(bundle.get("activity", []), on_select=_select_object)
...
def render_footer() -> None:
    st = _streamlit()
    st.markdown(
        '<div class="ovc-footer">OVC Research Console v0.3 · local fixture-only derived presentation · '
        'live projections denied · research writes denied · no selector, threshold, market, probability, exposure or execution authority</div>',
...
def run_console(identity: Mapping[str, Any] | None = None) -> None:
    ...
    with main:
        ...
        if workspace == "OVERVIEW":
            ...
            render_system(bundle)
        else:
            render_empty_state("BLOCK", "The selected workspace is not registered.", "Navigation fails closed.", "Return to Overview.")
