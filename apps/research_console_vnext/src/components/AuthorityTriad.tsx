...
export function AuthorityTriad({ available, authorised, active, reason }: AuthorityTriadProps) {
  ...
  return <div className={styles.authorityBlock} aria-label="Independent capability authority state">
    <div className={styles.authorityTriad}>{values.map(([label, value]) => <div key={label} className={value ? styles.authorityOn : styles.authorityOff} data-authority-state={`${label}:${value ? "YES" : "NO"}`}><span>{label}</span><strong>{value ? "YES" : "NO"}</strong></div>)}</div>
