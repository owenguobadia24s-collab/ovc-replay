...
export function InvestigationTabs(): JSX.Element {
  ...
  useEffect(() => {
    if (!active) return;
    if (requested !== active.investigation_id) { const next = new URLSearchParams(searchParams); next.set(INVESTIGATION_QUERY_KEY, active.investigation_id); setSearchParams(next, { replace: true }); }
    const nextIds = mergeLocalTabs(openIds, active.investigation_id);
    if (nextIds.join("|") !== openIds.join("|")) { setOpenIds(nextIds); window.localStorage.setItem(LOCAL_TAB_STORAGE_KEY, JSON.stringify(nextIds)); }
  }, [active, openIds, requested, searchParams, setSearchParams]);
  ...
  async function activate(item: Investigation): Promise<void> {
    const next = new URLSearchParams(searchParams); next.set(INVESTIGATION_QUERY_KEY, item.investigation_id); setSearchParams(next);
    setOpenIds((current) => { const updated = mergeLocalTabs(current, item.investigation_id); window.localStorage.setItem(LOCAL_TAB_STORAGE_KEY, JSON.stringify(updated)); return updated; });
    await Promise.all([queryClient.invalidateQueries({ queryKey: ["identity"] }), queryClient.invalidateQueries({ queryKey: ["capabilities"] }), queryClient.invalidateQueries({ queryKey: ["fixture-investigations"] })]);
  }
