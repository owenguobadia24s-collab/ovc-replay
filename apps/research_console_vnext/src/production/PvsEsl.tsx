...
export function ResearchModeLane({mode,generation}:{mode:"PATH_1"|"PATH_2";generation:string}) {return <EslCard node="244:10" kicker="DUAL-MODE RESEARCH" title={mode}><p>Provenance remains distinct until immutable ResearchCandidateGeneration {generation}.</p></EslCard>;}
...
export function C3BridgeMaturityState({state}:{state:"INTERFACE_ONLY"|"FIXTURE"|"SHADOW_EVALUATION"|"PRODUCTION_GRAMMAR"}) {const locked=state==="SHADOW_EVALUATION"||state==="PRODUCTION_GRAMMAR";return <EslRow node="244:88" label="C3 BRIDGE MATURITY" detail={state} state={locked?"LOCKED":"READY"}/>;}
...
export function EvidenceBalanceRow({claim,support,contradiction}:{claim:string;support:number;contradiction:number}) {return <EslRow node="249:115" label={claim} detail={`support=${support}; contradiction=${contradiction}; both visible`} state="READY"/>;}
