...
export function WorkspaceFrame() {
  ...
  const periodLabel = bars.length ? `${shortDate(bars[0]?.t)}  ${shortTime(bars[0]?.t)} → ${shortTime(bars.at(-1)?.t)}` : "FIXTURE WINDOW PENDING";
  return <section className={styles.workspace} data-density={density} data-rcn-ref="workspace" aria-label="Fixture-only synchronized operator workspace">
    ...
    <div className={styles.semanticFooter}>RCN-WP3E · reference-locked fixture reconstruction · Coverage is not confidence · selection, filters, density and command actions are presentation only.</div>
  </section>;
}
