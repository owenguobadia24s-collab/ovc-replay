...
`ReceiptStore` is content-addressed and append-only. Exact duplicate write is idempotent; same logical ID with divergent content is `VIT_LEDGER_INTEGRITY_FAIL`. Receipt resolution is by transaction/packet indexes derived from normative receipt bytes and rebuildable from the store.
