...
Status at freeze: `REVISED_2_REVIEW_CANDIDATE`  
...
Authority is resolved before MCAC execution. Missing or denied effective population or consumer authority blocks the IROF stage and emits only the typed IROF authority-failure receipt; MCAC emits no comparison or scientific result. For lawfully visible inputs, exactly one top-level outcome is chosen before relation evaluation:

1. invalid/tampered contract raises a typed contract error and emits no scientific record;
2. any attempted protected alias, including `TV120_NATIVE`/`2H_A_L`, raises a typed contract error and emits no scientific record;
3. incompatible already-authorised scopes, distinct clock coordinates lacking a lawful comparison domain, representation incompatibility, per-side generation mismatch, segment mismatch, or a gap/discontinuity crossing produces `NOT_COMPARABLE`;
4. missing owner, endpoint, required representation/evidence or dependency FVT produces `NOT_EVALUABLE`;
5. any dependency FVT after cutoff produces `NOT_EVALUABLE_FUTURE_DEPENDENCY`;
6. evidence valid only after the declared as-of time produces `RETROSPECTIVE_ONLY`;
7. censoring is carried explicitly; if it removes a required endpoint/evidence item it is `NOT_EVALUABLE`, otherwise evaluation proceeds with `censoring_state: PRESENT_BOUNDED`;
8. only then is the single primary relation computed.
...
- `TEMPORAL_CONTAINMENT`: earned only by primary relation `CONTAINS`, `STARTED_BY`, `FINISHED_BY`, `EQUAL_INTERVAL`, `STARTED_BY_POINT`, `CONTAINS_POINT`, or `FINISHED_BY_POINT`;
...
## 16. Revised 2 amendment closure map

Revised 1 incorporated design-review findings DR-01 through DR-09: exclusive point/interval algebra; all-dependency FVT; immutable coordinate versus mutable registry; executable gap/censoring/generation precedence; deterministic global correspondence; owner-published representation firewall; versioned doctrine/evidence roles; source-use/IROF authority crosswalk; and exact capacity failure law. Revised 2 closes the focused-review residuals by making the temporal-containment relation set identical in both normative passages, resolving authority denial before MCAC execution through IROF, reserving `NOT_COMPARABLE` for incompatible lawfully visible inputs, and routing protected alias attempts through typed contract rejection with no scientific result. The frozen original and Revised 1 remain preserved and are not ratified.
