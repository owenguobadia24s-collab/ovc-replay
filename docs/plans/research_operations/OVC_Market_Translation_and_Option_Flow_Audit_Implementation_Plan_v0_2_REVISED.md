...
**Status:** `PROPOSED_PENDING_MTA_G0_OPERATOR_RATIFICATION`

## Authority notice

This plan authorises repository design and bounded audit implementation only after operator ratification. It does not authorise provider intake, formula or threshold changes, clock replacement, active-selector mutation, C2E activation, C2.5 event promotion, C3 semantic promotion, R2 canonical publication, Validation consumption, probability, exposure, trading, execution or agent write authority.
...
- PR #209: open, unrelated and excluded
...
|---|---|---|---|
| MTA-00 | `gate/mta-g0-ratification` | baseline, authority, capacity, cluster, amendment, RO4 and June-review disposition packet | MTA-G0 |
...
The operator must explicitly decide:

1. ratify this audit-only plan;
2. set `PD-JUNE-FM-G2` to `DEFER` or `SUPERSEDE` without recording a blinded-review outcome;
3. preserve or close PR #202 without deleting its branch or external artifacts;
