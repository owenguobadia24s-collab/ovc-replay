...
def synthetic_stream(side: str, count: int):
    base = datetime(2021, 1, 1, tzinfo=timezone.utc)
    for index in range(count):
        group_index = index // 2
        timestamp = base + timedelta(minutes=15 * group_index)
        iso = timestamp.isoformat(timespec="seconds").replace("+00:00", "Z")
        suffix = "Z" if index % 2 == 0 else "A"
        yield {
            ...
            "source_record_id": f"RS0SOR-{side}-{group_index:08d}-{suffix}",
            "source_record_kind": "C2_LEVEL",
            "geometry_signature": {
                "horizon_id": "H4",
                "level_type": "RANGE_HIGH",
                "value": "1.25000",
                "origin": "C2AR_SYNTH_SOURCE_ORDER_RECOVERY",
...
def main() -> int:
    ...
    if args.rows <= 1:
        raise SystemExit("rows must be greater than one")

    bid_rows = args.rows // 2
    ask_rows = args.rows - bid_rows
    ...
    recovered_rows = merge_source_streams_with_tie_canonicalization(
        [
            synthetic_stream("BID", bid_rows),
            synthetic_stream("ASK", ask_rows),
        ]
    )
    ...
    receipt = {
        "schema": "ovc-c2p2-rs0-source-order-recovery-capacity-measurement/v1",
        "programme_id": "OVC-C2P2-RS0-SHADOW-EVIDENCE-v0.1",
        "packet_id": "C2P2-RS0-SOURCE-ORDER-RECOVERY",
        "measurement_class": "FULL_CARDINALITY_SYNTHETIC_NON_EVIDENTIARY_EQUAL_TIME_TIE_SHAPE",
        "status": status,
        "base_spooled_adapter_id": ADAPTER_ID,
        "source_order_adapter_id": SOURCE_ORDER_ADAPTER_ID,
        "candidate_id": CANDIDATE_A["candidate_id"],
        "synthetic_rows": args.rows,
        "synthetic_bid_rows": bid_rows,
        "synthetic_ask_rows": ask_rows,
