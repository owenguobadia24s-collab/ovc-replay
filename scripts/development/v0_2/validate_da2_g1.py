...
def main():
 ...
 for path,p in actual.items():
  ...
  if has:
   pr.add(path); assert 'concurrency:' in text and 'cancel-in-progress: true' in text, path
   if FULL in text: full.add(path)
 assert pr==CANONICAL, f'unexpected pull_request workflows: {sorted(pr-CANONICAL)}'
 assert full=={'.github/workflows/tests.yml'}, full
 ...
 print(f'DA2-G1 orchestration validation PASS; workflows={len(actual)}'); return 0
