...
class Q4StressReport:
    ...
    def passes(self) -> bool:
        return all((self.budget.lane_count >= 3,self.budget.min_packets_per_lane >= 2,self.no_lane_starvation,self.work_conserving,self.zero_false_authority_allows,self.reference_optimized_equivalence,self.safe_bypass_exercised,self.selective_invalidation_exercised,self.restart_exercised,self.external_reanchor_exercised))
...
def measure_q4(observations: Iterable[LanePacketObservation]) -> Q4StressReport:
    ...
    budget = VITOperationalBudget(len(by_lane),min(map(len,by_lane.values())),len(by_lane),len(obs),sum(i.discarded_for_topology for i in obs)/len(obs),mean(i.vit_lag_seconds for i in obs),max(i.vit_lag_seconds for i in obs),max(i.peak_memory_bytes for i in obs),max(i.durable_storage_bytes for i in obs),mean(i.checkpoint_seconds for i in obs),mean(i.rebuild_seconds for i in obs),sum(i.operator_intervention_unexpected for i in obs),sum(i.qualified_materialisable_idle_seconds for i in obs),severities["PLACEMENT_RECOMPUTE_ONLY"],severities["ASSURANCE_RENEWAL_REQUIRED"],severities["PAYLOAD_REBUILD_REQUIRED"],severities["AUTHORITY_REVIEW_REQUIRED"])
    counts=[len(v) for v in by_lane.values()]
    return Q4StressReport(budget,min(counts)>=2,not any(i.qualified_materialisable_idle_seconds>0 and not i.operator_intervention_unexpected for i in obs),not any(i.false_authority_allow for i in obs),all(i.reference_optimized_equal for i in obs),any(i.safe_bypass_exercised for i in obs),sum(severities.values())>0,any(i.restart_exercised for i in obs),any(i.external_reanchor_exercised for i in obs))
