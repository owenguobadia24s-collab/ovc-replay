...
from ovc.development.skills.vit_core import PacketIntegrationPayload, VitContractError, git_tree_sha
...
def run_isolated_rehearsal(
    ...
    *,
    ...
    tx = PhysicalMaterialisationTransaction(
        vit_generation_id=f"vit:{composition.receipt_id}",
    ...
    if crash_point == "BEFORE_WRITE":
        ...
        return IsolatedRehearsalResult("CRASH_BEFORE_WRITE", predecessor_commit, predecessor_tree, composition.result_tree, None, None, None, None, recovery)
    ...
    if crash_point == "POST_WRITE_PRE_RECEIPT":
        ...
        return IsolatedRehearsalResult("CRASH_POST_WRITE_PRE_RECEIPT", predecessor_commit, predecessor_tree, composition.result_tree, observed_commit, observed_tree, None, None, recovery)
    ...
    if not receipt_store_available:
        return IsolatedRehearsalResult("RECEIPT_STORE_UNAVAILABLE_STOP", predecessor_commit, predecessor_tree, composition.result_tree, observed_commit, observed_tree, None, None, "STOP_BEFORE_NEXT_TRANSACTION")
    receipt_store.put(receipt, receipt.receipt_id)

    if crash_point == "POST_RECEIPT_PRE_SUCCESSOR":
        return IsolatedRehearsalResult("CRASH_POST_RECEIPT_PRE_SUCCESSOR", predecessor_commit, predecessor_tree, composition.result_tree, observed_commit, observed_tree, receipt.receipt_id, None, "RECEIPT_RECOVERABLE")
    ...
    return IsolatedRehearsalResult("MATERIALISED_EQUIVALENT", predecessor_commit, predecessor_tree, composition.result_tree, observed_commit, observed_tree, receipt.receipt_id, completion.receipt_id, None)
