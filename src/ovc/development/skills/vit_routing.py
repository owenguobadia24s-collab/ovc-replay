...
PAYLOAD_LINEAGE_SCHEMA = "ovc-vit-payload-lineage/v2"
...
def _validated_pip(record: Mapping[str, Any]) -> tuple[str, str, str, str, str]:
    ...
    return programme_id, packet_id, route_class, authority_manifest_id, dependency_frontier_id


def build_vit_payload_lineage_record(
    *,
    programme_id: str,
    packet_id: str,
    ...
    """Build a stable payload-only VIT record with no physical-main placement.

    This is the forward late-binding form.  The PIP is durable; physical-main
    predecessor/result trees and queue position are intentionally absent until
    the candidate owns the final integration lease.
    """
...
def build_vit_lineage_record(
    *,
    ...
    """Build the historical v1 early-placement lineage.

    Preserved for historical replay and migration only.  New permanent PRs
    should use :func:`build_vit_payload_lineage_record`.
    """
    pip = dict(pip_identity_payload)
    if str(pip.get("programme_id", "")) != programme_id or str(pip.get("packet_id", "")) != packet_id:
        raise VitContractError("VIT_LINEAGE_PIP_PACKET_MISMATCH")
...
def _validate_payload_lineage(record: Mapping[str, Any], *, lineage_ref: str | None) -> ValidatedVitLineage:
    if record.get("status") != "PAYLOAD_ADMITTED":
        raise VitContractError("VIT_PAYLOAD_LINEAGE_NOT_ADMITTED")
    programme_id, packet_id, route_class, _, _ = _validated_pip(record)
    if record.get("binding_policy") != "LATE_PHYSICAL_PLACEMENT":
        raise VitContractError("VIT_PAYLOAD_BINDING_POLICY_INVALID")
    if any(key in record for key in ("generation", "generation_id", "placement", "placement_id")):
        raise VitContractError("VIT_PAYLOAD_EARLY_PLACEMENT_FORBIDDEN")
...
def _validate_v1_lineage(record: Mapping[str, Any], *, lineage_ref: str | None) -> ValidatedVitLineage:
    if record.get("status") != "ADMITTED":
        raise VitContractError("VIT_LINEAGE_NOT_ADMITTED")
    programme_id, packet_id, route_class, authority_manifest_id, dependency_frontier_id = _validated_pip(record)
    pip_id = str(record["pip_id"])
...
def validate_vit_lineage_record(record: Mapping[str, Any], *, lineage_ref: str | None = None) -> ValidatedVitLineage:
    """Validate either late-binding v2 payload lineage or preserved v1 lineage."""
    schema = record.get("schema")
    if schema == PAYLOAD_LINEAGE_SCHEMA:
        return _validate_payload_lineage(record, lineage_ref=lineage_ref)
    if schema == LINEAGE_SCHEMA:
        return _validate_v1_lineage(record, lineage_ref=lineage_ref)
    ...
    *,
    ...
    if not payload_changed and not dependency_frontier_changed and not authority_changed:
        return {
            ...
            "assurance_renewal_required": False,
        }
    return {
