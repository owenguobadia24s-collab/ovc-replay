...
FORBIDDEN_IDENTITY_FIELDS = frozenset({
    "family", "family_label", "C3_semantics", "c3_semantics",
    "OPT_C", "opt_c", "OPT_D", "opt_d",
    "outcome", "outcomes", "future_information",
    "Validation", "validation", "probability", "risk", "exposure",
    "trade_signal", "trading", "execution", "agent_write",
})
...
def _require_no_forbidden_fields(value: Any) -> None:
    if isinstance(value, Mapping):
        ...
        for child in value.values():
            _require_no_forbidden_fields(child)
...
def c2e_dependency_disposition(
    ...
    """Apply only currently frozen C2E dependency declarations.

    The present registry intentionally declares no episode-relative base role.
    Any future declaration is a new governed semantic contract and therefore
    fails closed here instead of inferring compatibility from episode identity,
    similarity, membership, or shared lineage.
    """
...
def evaluate_pair(
    ...
    *,
    ...
    if candidate_id.endswith("STRICT-CONTINUITY-v2"):
        ...
        pack_predicates = {
            "owner_declared_geometry_compatible": owner_compatible,
            "c2e_lineage_compatible_if_declared": c2e_disposition == "NOT_APPLICABLE_C2_ONLY",
        }
