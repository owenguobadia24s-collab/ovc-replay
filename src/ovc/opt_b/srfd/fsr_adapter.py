    ...
    for ledger in c2e_manifest["ledgers"]:
        for episode in ledger["episodes"]:
            ...
            for member_id in episode["member_record_ids"]:
                ...
                sequence.append(
                    {
                        "motion_price_delta": structural["motion_price_delta"],
                        "measurement_container_width": structural["measurement_container_width"],
                    }
                )
            if not sequence:
                continue
            records.append(
                {
                    ...
                    "sequence": sequence,
                    "structural": {
                        "duration_count": len(sequence),
                        "censored": 1 if episode["status"] == "CENSORED" else 0,
                    },
                    "parent_context": None,
                    "instrument": "GBPUSD",
                    "side": ledger["side"],
...
def _representation_sets(
    c2_manifest: Mapping[str, Any], c2e_manifest: Mapping[str, Any]
    ...
    normalization = fit_minmax_normalization(
        fit_records,
        STATIC_FIELDS,
        fit_population_id=static_population["population_id"],
        fit_cutoff=fit_cutoff,
    )

    packs = {
        "R1": RepresentationPack("FSR.SRFD.R1.RAW_C2.v1", "SRFDI-R1", "R0", STATIC_FIELDS, "FSR.STATIC", "STATIC_VECTOR"),
        "R2": RepresentationPack("FSR.SRFD.R2.C2E_AGG.v1", "SRFDI-R2", "R3", ("motion_price_delta", "measurement_container_width"), "FSR.EPISODE_AGG", "STATIC_VECTOR"),
    ...
    for source in static_records:
        for key in ("R8", "R9"):
            rep = compile_representation(source, packs[key], source_population_id=static_population["population_id"])
            sets[key].append(_decorate(rep, source, f"FSR_{key}_V1"))
        if source["computability_status"] != "EVALUABLE":
            continue
        for key in ("R1", "R6", "R7"):
            rep = compile_representation(source, packs[key], source_population_id=static_population["population_id"])
            sets[key].append(_decorate(rep, source, f"FSR_{key}_V1"))
        rep = compile_representation(source, packs["R4"], source_population_id=static_population["population_id"], normalization_pack=normalization)
        sets["R4"].append(_decorate(rep, source, "FSR_R4_V1"))

    for source in episode_records:
        for key in ("R2", "R3", "R5"):
            rep = compile_representation(source, packs[key], source_population_id=episode_population["population_id"])
            sets[key].append(_decorate(rep, source, f"FSR_{key}_V1"))
    ...
    return {key: sorted(value, key=lambda item: item["representation_id"]) for key, value in sets.items()}, metadata
...
def _family_benchmark(
    normalized_representations: Sequence[Mapping[str, Any]],
    normalized_distances: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    ...
    for side in sorted(by_side):
        ...
        for radius in ("0.15", "0.30"):
            configs = [
                FamilyMethodSpec("GREEDY_LEXICOGRAPHIC_MEDOID_STAR", f"FSR.{side}.STAR.R{radius}", radius=radius, minimum_support=2),
                FamilyMethodSpec("COMPLETE_LINKAGE", f"FSR.{side}.COMPLETE.R{radius}", radius=radius, minimum_support=2, linkage="complete"),
                FamilyMethodSpec("AVERAGE_LINKAGE", f"FSR.{side}.AVERAGE.R{radius}", radius=radius, minimum_support=2, linkage="average"),
            ]
            catalogs.append(medoid_star(matrix, configs[0]))
            catalogs.append(hierarchical(matrix, configs[1]))
            catalogs.append(hierarchical(matrix, configs[2]))
    ...
    for catalog in catalogs:
        side = str(catalog["configuration_id"].split(".")[1]) if "." in str(catalog["configuration_id"]) else "UNKNOWN"
        grouped_catalogs[side].append(catalog)
...
def run_fsr_srfd(c2_manifest: Mapping[str, Any], c2e_manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    specs = {
        ...
        "R2_L1": DistanceSpec("FSR.DIST.R2.L1.v1", "L1_TYPED", ("motion_price_delta_mean", "measurement_container_width_mean", "duration_count")),
    ...
    mapping = {
        "R1_L1": "R1", "R1_L2": "R1", "R1_GOWER": "R1", "R2_L1": "R2",
        "R3_DTW": "R3", "R4_L1": "R4", "R5_DTW": "R5", "R6_L1": "R6",
        "R7_L1": "R7", "R9_GOWER": "R9",
    }
    ...
    body = {
        ...
        "distance_status_counts": {
            key: dict(sorted(Counter(item["status"] for item in value).items()))
