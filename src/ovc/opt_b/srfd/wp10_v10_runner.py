...
def preflight_wp10_v10(
    *, binding: RunBindingV10, source_paths: Mapping[str, str | Path],
    ...
    body = {
        ...
        "storage_layout": "CONTENT_ADDRESSED_CHUNKED_COMPRESSED_SHARED_INFRASTRUCTURE", "provider_fetch": "DENIED",
...
def _seed_verified_v09_prefix(
    *, new_start: RunStartReceipt, binding: RunBindingV10, v09_root: Path,
    ...
    old artifact hash, re-materialises unchanged outputs in the v1.0 shared CAS adapter, then writes
    a NEW v1.0 checkpoint. It never copies or relabels an old checkpoint.
    """
    ...
    for old_receipt in old_latest.unit_receipts[:prefix]:
        imported.append(artifact_store.import_v09_output(
            start=new_start, binding=binding, v09_root=Path(v09_root), source=source,
