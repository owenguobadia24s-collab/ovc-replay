...
class RunBindingV11:
    ...
    def to_dict(self) -> dict[str, str]:
        ...
        for name in (
            ...
            "capacity_envelope_sha256", "storage_binding_sha256",
            "execution_environment_profile_sha256", "hardening_qa_sha256",
            "execution_binding_sha256",
        ):
            out[name] = _hex64(out[name], name)
        if (
            out["programme_id"] != PROGRAMME_ID
            or out["packet_id"] != PACKET_ID
            or out["population_id"] != FROZEN_POPULATION_ID
        ):
            raise WP10V11InterfaceError("V11_IDENTITY_DRIFT", f"{out['programme_id']}:{out['packet_id']}:{out['population_id']}")
        return out
