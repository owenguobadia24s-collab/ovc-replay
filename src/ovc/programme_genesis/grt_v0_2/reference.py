...
def _artifact_type_from_path(path: str) -> str | None:
    ...
    if p.startswith(".github/workflows/") and p.endswith((".yml", ".yaml")):
        return "WORKFLOW"
    root_types = {
        "src": "IMPLEMENTATION", "apps": "IMPLEMENTATION", "contracts": "CONTRACT", "schemas": "SCHEMA",
        "registries": "REGISTRY", "fixtures": "FIXTURE", "tests": "TEST", "scripts": "TOOLING", "tools": "TOOLING",
        "artifacts": "GENERATED_ARTIFACT", "records": "DOCUMENTATION", "plans": "PLAN", "legacy": "DOCUMENTATION",
    }
...
def observe_component(*, tree_hash: str, path: str, content_hash: str, component_type: str = "file") -> dict[str, Any]:
    ...
    return {"component_id": "GRT.OBS." + canonical_sha256(identity)[:24], **identity, "scanner_version": "GRT-REFERENCE-WP3A.v1", "observed_relationships": []}
...
def classify_observation(observation: Mapping[str, Any], binding: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    for rel in binding.get("relationships", []):
        ...
        relationships.append({"relationship_type": rel["relationship_type"], "object_artifact_id": rel["object_artifact_id"], "evidence_status": rel.get("evidence_status", "SOURCE_EXPLICIT"), "evidence_refs": sorted(set(rel.get("evidence_refs", []))), "authority_effect": rel.get("authority_effect", "NONE")})
    relationships.sort(key=lambda item: (item["relationship_type"], item["object_artifact_id"]))
    return {"artifact_id": artifact_id, "artifact_type": artifact_type, "lifecycle_class": lifecycle, "lifecycle_source": lifecycle_source, "physical_components": [observation["component_id"]], "logical_namespace": binding.get("logical_namespace"), "programme_binding": binding.get("programme_binding"), "genesis_binding": binding.get("genesis_binding"), "declared_relationships": relationships, "evidence": [observation["component_id"]], "constitution_version": "v0.2", "artifact_status": status, "binding_status": "SOURCE_EXPLICIT" if binding else "CANDIDATE_RELATION"}
...
def build_reference_graph(*, tree_hash: str, components: Sequence[Mapping[str, Any]], bindings_by_path: Mapping[str, Mapping[str, Any]] | None = None) -> dict[str, Any]:
    ...
    for raw in sorted(components, key=lambda item: _path(str(item.get("path", "")))):
        ...
        observations.append(obs); art = classify_observation(obs, bindings.get(obs["path"])); grouped[art["artifact_id"]].append(art)
    ...
    for artifact_id in sorted(grouped):
        ...
        combined = dict(exemplar); combined["physical_components"] = sorted({x for item in group for x in item["physical_components"]}); combined["evidence"] = sorted({x for item in group for x in item["evidence"]})
        if combined["artifact_status"] != "RESOLVED": unresolved.append(artifact_id)
        artifacts.append(combined)
    body = {"schema": "grt-repository-reference-graph/v0.2", "runtime_release": "GRT-REFERENCE-WP3A.v1", "serialization_profile": SERIALIZATION_ID, "tree_hash": tree_hash, "observed_components": observations, "repository_artifacts": artifacts, "unresolved_artifact_ids": sorted(unresolved), "resolution_status": "RESOLVED" if not unresolved else "PARTIAL", "authority_effect": "NONE_REFERENCE_OBSERVATION_ONLY", "active_enforcement": "NONE"}
    return {**body, "canonical_hash": canonical_sha256(body)}


def scan_repository_tree(repository_root: Path | str, *, commit: str, bindings_by_path: Mapping[str, Mapping[str, Any]] | None = None) -> dict[str, Any]:
    """Adapter over the existing v0.1 exact `git ls-tree` inventory substrate."""
    root = Path(repository_root)
    if not _HEX40.fullmatch(commit): raise ReferenceRuntimeError("GRT_REFERENCE_COMMIT_INVALID")
    completed = subprocess.run(["git", "-C", str(root), "rev-parse", f"{commit}^{{tree}}"], check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", errors="replace")
    if completed.returncode != 0: raise ReferenceRuntimeError("GRT_REFERENCE_TREE_RESOLUTION_FAILED")
    tree_hash = completed.stdout.strip()
    if not _HEX40.fullmatch(tree_hash): raise ReferenceRuntimeError("GRT_REFERENCE_TREE_INVALID")
    inventory = tracked_inventory(root, commit=commit)
    components = [{"path": row["path"], "content_hash": row["blob_hash"], "component_type": "file"} for row in inventory]
...
def replay_b0_baseline(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    validate_baseline_members(rows); membership_hash = baseline_membership_sha256(rows)
    if len(rows) != B0_MEMBER_COUNT or membership_hash != B0_MEMBERSHIP_SHA256: raise ReferenceRuntimeError("GRT_REFERENCE_B0_REPLAY_MISMATCH")
    return {"schema": "grt-reference-b0-replay/v0.2", "runtime_release": "GRT-REFERENCE-WP3A.v1", "member_count": len(rows), "membership_sha256": membership_hash, "pending_mapping_count": sum(1 for row in rows if row.get("mapping_status") != "MAPPED"), "authority_effect": "NONE_HISTORICAL_REPLAY_ONLY", "active_enforcement": "NONE", "status": "PASS"}
