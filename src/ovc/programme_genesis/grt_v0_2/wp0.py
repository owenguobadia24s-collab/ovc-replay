    ...
    with tempfile.TemporaryDirectory(prefix="grt2-wp0-") as temp_root_text:
        ...
        try:
            ...
            source_commit = str(
                ...
                or read_model.get("portfolio", {}).get("source_commit")
                or ""
            )
...
def reconcile(repository_root: Path, *, baseline_commit: str, verify_b0_determinism: bool = True) -> dict[str, Any]:
    ...
    repository_root = repository_root.resolve()
    # Pull-request merge checkouts are intentionally shallow by default. Fetch
    # the exact court-record baseline before resolving it so WP0 does not
    # confuse checkout topology with source non-reproducibility.
    ensure_commit(repository_root, baseline_commit)
