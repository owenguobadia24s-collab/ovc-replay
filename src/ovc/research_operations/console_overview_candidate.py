...
def load_read_model(path: Path) -> ResearchReadModel:
    ...
    The repository build script historically owned this helper even though the
    acceptance surface imports it from the candidate module. Keep one stable
    library-level loader so both pytest and external callers can resolve the
