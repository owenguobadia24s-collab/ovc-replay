...
GENERATION_ID = "OVC-LSIAC-ACCESSION-GEN-0001"
...
def load_source_passports(root: str | Path) -> list[dict[str, Any]]:
    """Reconstruct every pre-adjudication source passport from the frozen codebooks.

    This function performs source-state reconstruction only. It makes no
    inheritance, role, lifecycle, destination, architecture-effect, or promotion
    decision.
    """
    root = Path(root)
...
def _aggregate_subject(
    ...
    source_relation = "MULTIPLE_STANDING" if len(set(standings)) > 1 or "MULTIPLE_STANDING" in relation_values else "CONSISTENT"
    ...
    if delta_entry is not None:
        ...
        scientific_disposition = candidate_disposition if candidate_disposition in ALLOWED_DISPOSITIONS else "UNRESOLVED"
    ...
    triage_class = "SOURCE_BLOCKED" if source_standing in {"PENDING_SOURCE_BINDING", "SOURCE_CONFLICT"} else "FULL_INDIVIDUAL_REVIEW"

    rationale = (
        "Deterministic Pass-1 projection from the frozen source-passport set"
        + (" and exact post-v0.5 delta entry" if delta_entry is not None else "")
        + "; unresolved scientific standing is preserved rather than inferred from phase names, recurrence, status labels, or the initial inheritance docket."
    )
    return {
...
def build_shared_locator_dependence_graph(
    passports: list[Mapping[str, Any]],
) -> dict[str, Any]:
    """Build exact shared-locator dependence edges only.

    Absence of an edge never implies independence. The frozen protocol requires
...
def build_virtual_view_identity(*, algorithm_git_blob_sha: str) -> str:
    """Bind the deterministic Pass-1 view without requiring a giant checked-in row dump."""
    payload = {
        ...
        "scope": "ALL_432_FROZEN_ACCESSION_SUBJECTS",
    }
