...
def _paired(left, right):
    ...
    return [(Decimal(str(a)), Decimal(str(b))) for a, b in zip(left, right) if a is not None and b is not None]
...
def pearson(left, right) -> Decimal | None:
    ...
    if n < 2:
        return None
    # Use an isolated deterministic precision for this calculation only. Never mutate
    # the process-global Decimal context because that can change unrelated OVC contracts/tests.
