...
def _validate_correspondence_series_root(
    ...
    *,
    ...
    A deterministic first generation is self-proving under the frozen Series identity
    formula. A successor generation must carry the canonical identity history needed
    by the existing reference reconciler to prove its Series first-generation root.
    """
...
def install_reference_series_root_guard(reference_module: Any) -> None:
    """Install the bounded remediation-4 guard on the reference correspondence entrypoint."""
