...
class CiPerformanceG4GateTests(unittest.TestCase):
    ...
    def test_gate_is_operator_required_and_recommends_bounded_shadow_sharding(self):
        ...
        self.assertEqual(self.gate["recommended_option"], "B_DETERMINISTIC_REQUIRED_SHARD_UNION_SHADOW")
        self.assertIn("DEFER", self.gate["decision_options"]); self.assertIn("BLOCK", self.gate["decision_options"])

    def test_no_implementation_candidate_exists_in_gate_packet(self):
        self.assertEqual(self.gate["candidate_commit"], "NOT_CREATED_OPERATOR_APPROVAL_REQUIRED")
        self.assertEqual(self.qa["implementation_candidate_commit"], "NOT_CREATED_OPERATOR_APPROVAL_REQUIRED")
        self.assertIn("activate the shard candidate as a substitute for any existing required check", self.gate["proposed_authority_delta"]["not_authorised_by_this_pass"])

    def test_current_required_python_assurance_is_preserved_under_siq_timing(self):
    ...
    def test_gate_prohibits_test_weakening_and_runner_cutover(self):
        ...
        self.assertIn("change benchmark freshness, scientific evidence semantics or heavy-test assertions", denied)
        self.assertEqual(self.pyt["compatibility_result"], "PASS_IF_RUNNER_NEUTRAL_SHADOW_SHARDING_ONLY")
    ...
    def test_state_preserves_no_topology_activation_across_gate_transition(self):
        self.assertEqual(self.state["packet_id"], "CIPR-WP4"); self.assertFalse(self.state["topology_change_active"])
        if self.state["status"] == "GATE_READY":
            self.assertEqual(self.state["operator_stop_gate"], "CIPR-G4-SUITE-TOPOLOGY"); self.assertEqual(self.state["authority_required"], "OPERATOR_REQUIRED"); self.assertIsNone(self.state["decision_record"])
        elif self.state["status"] == "APPROVED":
            self.assertIsNone(self.state["operator_stop_gate"]); self.assertEqual(self.state["authority_required"], "SATISFIED_OPERATOR_PASS")
            self.assertEqual(self.state["decision_record"], "docs/releases/ci-performance-remediation-v0-1/cipr-wp4/CIPR_G4_DECISION.json")
