...
class CiPerformanceShadowPacketTests(unittest.TestCase):
    ...
    def test_operator_pass_exactly_authorizes_shadow_option_b(self):
        ...
        self.assertEqual(self.decision["selected_option"], "B_DETERMINISTIC_REQUIRED_SHARD_UNION_SHADOW")
        self.assertIn("activate shard output as a substitute for any existing required check", self.decision["authority_not_granted"])

    def test_policy_is_four_way_deterministic_and_isolates_known_heavy_paths(self):
        self.assertEqual(self.policy["schema"], "ovc-unittest-shard-policy/v1"); self.assertEqual(self.policy["shard_count"], 4)
        self.assertEqual(self.policy["assignment_algorithm"], "LEGACY_DISCOVERY_ORDER_HEAVY_PATH_ISOLATION_THEN_ROUND_ROBIN")
        heavy = self.policy["heavy_path_to_shard"]; self.assertEqual(len(heavy), 4); self.assertEqual(set(heavy.values()), {0,1,2,3})

    def test_shadow_workflow_is_not_a_third_pull_request_listener(self):
        self.assertNotIn("pull_request:", self.workflow); self.assertIn("build/ci-performance-shadow-shards", self.workflow)
        self.assertIn("workflow_dispatch:", self.workflow); self.assertIn("matrix:", self.workflow); self.assertIn("shard: [0, 1, 2, 3]", self.workflow)
        self.assertIn("unittest_shard_shadow.py prove", self.workflow); self.assertIn("unittest_shard_shadow.py run", self.workflow); self.assertNotIn("xdist", self.workflow)

    def test_existing_required_checks_are_preserved_with_siq_minimal_critical_section(self):
        ...
        self.assertIn("PREDECESSOR_MOVED", self.resolver)
        self.assertIn("prospective_result_tree", self.resolver)
        self.assertNotIn("OVC_RECONCILE_REQUIRED", self.resolver)
    ...
    def test_packet_state_preserves_non_activation_boundary(self):
        ...
        self.assertFalse(self.state["required_check_substitution_active"]); self.assertFalse(self.state["runner_cutover_active"]); self.assertFalse(self.state["scientific_authority_delta"])
