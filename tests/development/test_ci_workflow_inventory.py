...
EXPECTED_SNAPSHOTS = {
    "0_10": (191, 140, 51), "0_9": (191, 139, 52), "0_8": (191, 138, 53),
    "0_7": (191, 137, 54), "0_6": (191, 136, 55), "0_5": (191, 135, 56),
    "0_4": (191, 134, 57), "0_3": (191, 133, 58), "0_2": (177, 131, 46), "0_1": (175, 129, 46),
}
...
class CiWorkflowInventoryGovernanceTests(unittest.TestCase):
    ...
    def test_current_inventory_is_exhaustive_and_exact(self):
        ...
        self.assertEqual(self.inventory["total_workflow_definitions"], 140)
        self.assertEqual(sum(self.inventory["category_counts"].values()), 140)
    ...
    def test_r2_qualifier_is_push_only_synthetic_and_fail_closed(self):
        admission = self.policy["admitted_new_workflow"]
        path = ".github/workflows/c2p2-sd-wp3-qualification-r2.yml"
        ...
        for marker in ("SYNTHETIC_ONLY", "NO_REAL_SOURCE_READ", "NO_REAL_SOURCE_EXECUTION", "NO_SELECTION", "NO_ACTIVATION", "NO_VALIDATION", "NO_PUBLICATION"):
            self.assertIn(marker, admission["authority_mode"])

    def test_original_wp3_qualifier_and_r5_are_preserved_non_pr(self):
        admissions = {row["path"]:row for row in self.policy["additional_non_pr_diagnostic_workflows"]}
        for path in (".github/workflows/c2p2-sd-wp3-qualification.yml", ".github/workflows/c2p2-rs0-real-source-shadow-run-r5.yml"):
