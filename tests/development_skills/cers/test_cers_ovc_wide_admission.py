...
def test_operator_pass_materialises_exact_admission_delta():
    ...
    assert decision["operator_phrase"] == "OVC APPROVE CERS-G-OVC-WIDE-ADMISSION-1 PASS  and shared systems"
    assert decision["operator_decision_time"] == "2026-08-21T17:21:00+01:00"
...
def test_root_registry_and_admission_registry_are_exact_and_exhaustive():
    ...
    assert set(root_map) == {
        ...
        "GRT_V0_2", "PRVITR", "P2CTI", "ASOCSI", "SHARED_SYSTEMS",
    }
...
def test_each_new_admission_preserves_its_reserved_boundary():
    ...
    assert shared["current_state_root"] == "docs/programmes/shared-systems-v0-1/programme_state/SHSI_PROGRAMME_STATE_v0_2_R1.json"
    assert shared["governing_plan_id"] == "OVC-SHARED-SYSTEMS-CONFORMANCE-IMPLEMENTATION-PLAN-0.2-R1"
    assert "ACTIVE_SHARED_SYSTEMS_RUNTIME" in shared["explicit_prohibitions"]
    assert "CURRENT_CONSUMER_CUTOVER" in shared["explicit_prohibitions"]
    assert "FROZEN_CONTRACT_CHANGE" in shared["explicit_prohibitions"]
    assert "SCOPE_OR_TEMPO_EXPANSION" in shared["explicit_prohibitions"]
    assert "INVARIANT_OVERRIDE" in shared["explicit_prohibitions"]
...
def test_cers_state_advances_append_only_and_keeps_persistent_run_bounded():
    ...
    assert current["admission_expansion_gate"]["effective_admitted_programme_count"] == 5
    assert current["admission_expansion_gate"]["operator_required_boundaries"] == "PARK"


def test_census_records_existing_work_as_reconcile_not_duplicate_start():
    census = load(
        "docs/releases/development-skills-v0-3/cers-persistent-supervisor/admission/"
        "CERS_OVC_WIDE_ADMISSION_CENSUS_v0_1.json"
    )
    work = {row["programme_id"]: row for row in census["work_reconciliation"]}
    assert work["OVC-P2CTI-CONFORMANCE-v0.1"]["pr"] == 1247
    assert work["OVC-P2CTI-CONFORMANCE-v0.1"]["status"] == "MERGED"
    assert work["OVC-ASOCS-6M-v0.1"]["pr"] == 1253
    assert work["OVC-ASOCS-6M-v0.1"]["status"] == "MERGED"
    assert work["OVC-GRT-V0.2-REPOSITORY-CONSTITUTION-CONTINUOUS-CONFORMANCE"]["pr"] == 1252
    assert work["OVC-SHARED-SYSTEMS-v0.1"]["pr"] == 978
    assert all("NO_DUPLICATE_START" in row["rule"] for row in work.values())
