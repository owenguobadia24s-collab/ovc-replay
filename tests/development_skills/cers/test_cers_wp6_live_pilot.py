...
def test_observed_pilot_record_binds_exact_dispatch_and_background_assurance():
    run = load(RUN_PATH)
    assert run["status"] in {"START_ACKNOWLEDGED_LIVE_BRANCH_WRITE_OBSERVED", "COMPLETED_BRANCH_PILOT_BACKGROUND_ASSURANCE_STILL_RUNNING", "COMPLETED_PASS"}
    ...
    if run["status"] == "COMPLETED_BRANCH_PILOT_BACKGROUND_ASSURANCE_STILL_RUNNING":
        assert run["heartbeat_sequence"] == 2
        assert run["background_assurance"]["status_at_branch_pilot_completion"] == "IN_PROGRESS"
        assert run["background_assurance"]["full_repository_suite_step_at_completion"] == "IN_PROGRESS"
