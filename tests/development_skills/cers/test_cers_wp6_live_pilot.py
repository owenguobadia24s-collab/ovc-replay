...
def identity(generation: int = 1) -> DispatchIdentity:
    work = {
    ...
    return DispatchIdentity(PROGRAMME_ID, PACKET_ID, "WRITE_FILE", canonical_id(work), EXECUTOR_ID, generation)
        ...
        "workflows": [
            {
                ...
                "run_number": background["run_number"],
                "status": "in_progress",
            }
        ],
    }
...
def test_exact_live_actions_allow_only_frozen_packet_branch_scope():
    c = coordinator()
    allowed = c.authorize_action(action="WRITE_FILE", branch=c.branch, semantic_owner="CERS", path=FIRST_WRITE)
    ...
    with pytest.raises(LivePilotViolation):
        c.authorize_action(action="WRITE_FILE", branch="main", semantic_owner="CERS", path=FIRST_WRITE)
    with pytest.raises(LivePilotViolation):
        c.authorize_action(action="WRITE_FILE", branch=c.branch, semantic_owner="CERS", path="README.md")
    with pytest.raises(LivePilotViolation):
        c.authorize_action(action="WRITE_FILE", branch=c.branch, semantic_owner="OTHER", path=FIRST_WRITE)


def test_real_unattended_start_requires_background_assurance_and_caller_absence():
    c = coordinator()
    lease = c.acquire_lease()
    ...
    no_background = witness()
    no_background["workflows"][0]["status"] = "completed"
    c2 = coordinator()
    lease2 = c2.acquire_lease()
    ...
    no_caller_absence = witness()
    no_caller_absence["caller_absent"] = False
...
def test_stale_fence_and_stale_worker_completion_fail_closed():
    ...
    old_worker = c.workers[d1.dispatch_id]
    restored.transactions[d1.dispatch_id] = c.transactions[d1.dispatch_id]
    restored.workers[d1.dispatch_id] = old_worker
...
def test_unknown_start_reconciles_existing_worker_without_blind_redispatch():
    ...
    lease = c.acquire_lease()
    d = identity()
    unknown = c.mark_unknown_start(d, lease)
    assert unknown.phase == "DISPATCH_UNKNOWN"
    ...
    assert resolved.phase == "START_ACKNOWLEDGED"
    assert resolved.reason == "RECOVERED_EXISTING_START_NO_REDISPATCH"
    assert c.last_reconciliation_id


def test_quiescence_rollback_blocks_new_starts_and_checkpoint_is_zero_chat():
    c = coordinator()
    lease = c.acquire_lease()
    c.disable_new_dispatch()
    ...
    checkpoint = c.checkpoint()
    assert checkpoint.chat_dependency_count == 0
    restored = LivePilotCoordinator.restore(c.authority, c.freeze, checkpoint)
...
def test_action_registry_has_no_merge_direct_main_force_or_unknown_allow():
    ...
    by_action = {row["action"]: row for row in registry["entries"]}
    assert set(by_action) == {"WRITE_FILE", "GIT_COMMIT", "PUSH_BRANCH"}
    assert all(row["allowed"] for row in by_action.values())
    assert "MERGE" in registry["explicit_denies"]
    assert "DIRECT_MAIN_WRITE" in registry["explicit_denies"]
    assert "FORCE_PUSH" in registry["explicit_denies"]
    assert registry["unknown_action"] == {"side_effect_class": "IRREVERSIBLE_OR_UNKNOWN", "decision": "DENY"}


def test_observed_pilot_record_binds_exact_dispatch_and_background_assurance():
    run = load(RUN_PATH)
    assert run["status"] in {"START_ACKNOWLEDGED_LIVE_BRANCH_WRITE_OBSERVED", "COMPLETED_PASS"}
    assert run["programme_id"] == PROGRAMME_ID and run["packet_id"] == PACKET_ID
    assert run["executor_identity"] == EXECUTOR_ID
    assert run["dispatch_id"] == identity().dispatch_id
    assert run["worker_concurrency"] == 1 and run["max_speculative_depth"] == 1
    assert run["background_assurance"]["status_at_start"] == "IN_PROGRESS"
    assert run["caller_absent_at_start"] is True
    assert run["semantic_actions"] == ["WRITE_FILE", "GIT_COMMIT", "PUSH_BRANCH"]
    assert run["direct_main_mutation"] is False
    assert run["merge_attempted"] is False
