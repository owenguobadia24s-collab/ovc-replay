...
class DsaiVitV03Wp2Tests(unittest.TestCase):
    ...
    def test_reference_add_modify_delete_is_deterministic(self) -> None:
        b = self._blob("b\n")
        add = self._pip(({"op":"ADD","path":"b.txt","blob_sha":b,"mode":"100644"},))
        receipt1 = apply_payload_reference(self.repo, self.base_tree, add)
        receipt2 = apply_payload_reference(self.repo, self.base_tree, add)
        ...
        self.assertEqual(receipt1.disposition, "COMPOSED")

        modified = self._blob("changed\n")
        mod_receipt = apply_payload_reference(self.repo, self.base_tree, self._pip(({"op":"MODIFY","path":"a.txt","blob_sha":modified,"mode":"100755"},)))
        self.assertFalse(mod_receipt.failures)
        self.assertNotEqual(mod_receipt.result_tree, self.base_tree)

        del_receipt = apply_payload_reference(self.repo, self.base_tree, self._pip(({"op":"DELETE","path":"a.txt"},)))
        self.assertFalse(del_receipt.failures)
    ...
    def test_binary_and_symlink_modes_are_supported_by_exact_blob_identity(self) -> None:
        binary = subprocess.check_output(["git","-C",str(self.repo),"hash-object","-w","--stdin"], input=b"\x00\xff\x10")
        binary_sha = binary.decode().strip()
        binary_receipt = apply_payload_reference(self.repo, self.base_tree, self._pip(({"op":"ADD","path":"blob.bin","blob_sha":binary_sha,"mode":"100644"},)))
        self.assertFalse(binary_receipt.failures)
        link_sha = self._blob("a.txt")
        link_receipt = apply_payload_reference(self.repo, self.base_tree, self._pip(({"op":"ADD","path":"link","blob_sha":link_sha,"mode":"120000"},)))
        self.assertFalse(link_receipt.failures)

    def test_duplicate_invalid_object_and_unsafe_path_fail_closed(self) -> None:
        b = self._blob("b\n")
        ...
        bad = apply_payload_reference(self.repo, self.base_tree, self._pip(({"op":"ADD","path":"y","blob_sha":"f"*40,"mode":"100644"},)))
        self.assertIsNone(bad.result_tree)
        self.assertEqual(bad.failures[0].reason_code, "INPUT_PRECONDITION_MISMATCH")
