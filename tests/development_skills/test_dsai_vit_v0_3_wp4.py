...
class DsaiVitV03Wp4Tests(unittest.TestCase):
    def test_state_drainage_reconstructs_without_chat_or_process_memory(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            store = DurableExecutionStore(td)
    ...
    def test_corrupted_durable_state_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            state = PersistentExecutionState(
                ContinuousExecutionMandate("P","WP4","CONTINUE","CONTINUE_UNTIL_MANDATORY_STOP","plan"),
                DevelopmentLane("lane","P","WP4","build"),
                RecoveryState("RUNNING",next_packet="WP5"),
            )
