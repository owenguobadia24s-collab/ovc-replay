...
class DsaiVitV03Wp8BTests(unittest.TestCase):
    def _obs(self):
        ...
        for lane in ("L1","L2","L3"):
            for i in (1,2):
                rows.append(LanePacketObservation(
                    lane, f"{lane}-P{i}", 1.0+i, 0.2, 0.5+i, 0.1,
                    1000+i, 2000+i,
                    "PLACEMENT_RECOMPUTE_ONLY" if i == 1 else "ASSURANCE_RENEWAL_REQUIRED",
                    safe_bypass_exercised=(lane == "L1" and i == 2),
                    restart_exercised=(lane == "L2" and i == 2),
                    external_reanchor_exercised=(lane == "L3" and i == 2),
                ))
        return tuple(rows)
    ...
    def test_three_lane_two_packet_measured_report_passes(self) -> None:
        ...
        self.assertEqual(report.budget.false_parallel_value_rate,0.0)
