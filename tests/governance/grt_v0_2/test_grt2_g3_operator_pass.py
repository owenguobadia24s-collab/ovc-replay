...
def test_generation_zero_floor_is_exact_approved_superseding_object():
 decision=load(G3/"GRT2_G3_TERMINAL_SUPERSESSION_DECISION.json"); proposed=dict(decision["proposed_replacement_generation_0_floor"]); proposed.pop("member_count"); proposed.pop("status"); active=load(GOV/"debt_floors/GRT_DEBT_FLOOR_G0.json"); assert active==proposed; validate_debt_floor(active); assert len(active["open_grandfathered_findings"])==1648; assert active["floor_hash"]=="2c2152397e1ac5ace98b3363ca39c84f5d5a5dadbc6243e73cbd1fba15413c8b"
