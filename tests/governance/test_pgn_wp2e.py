...
EI_PROGRAMME = "OVC-MARKET-GRAMMAR-EMPIRICAL-INTEGRATION-JUNE-v0.1"
...
class NativeGenesisFrozenSnapshotAndAdmissionTests(unittest.TestCase):
    ...
    def test_snapshot_manifest_identity_and_counts_are_immutable(self) -> None:
        ...
        self.assertEqual("b3fe76028609c7ab45b0779411df8206f0aaf22eecaaf6ff20f4106f49387b68", self.manifest["bundle_manifest_sha256"])
        self.assertEqual("0ecec2bed3094cf659e25a1c74612d64ae10aec6f0c22114f4be6b11747c3584", self.manifest["child_file_identity_sha256"])
        self.assertEqual("d562b7427ab0c306df04e2472a517d1b0bceb8b7", self.snapshot["manifest_git_blob_sha1"])
    ...
    def test_snapshot_objects_are_unique_and_not_rebuilt_from_later_files(self) -> None:
        ...
        self.assertNotIn(EI_PROGRAMME, ids)
    ...
    def test_snapshot_exclusions_lineage_and_prior_census_are_preserved(self) -> None:
        exclusions = {item["object_id"] for item in self.snapshot["exclusion_ledger"]}
    ...
    def test_sealed_sixteen_candidate_population_is_unchanged(self) -> None:
        ...
        self.assertEqual("7dad0cee63ba6d533eff097f333645914e92d87e1964634786312f9ede58354a", self.candidates["candidate_set_sha256"])
        candidate_ids = {item["programme_id"] for item in self.candidates["candidate_commitments"]}
        self.assertEqual(16, len(candidate_ids))
        self.assertNotIn(MG_PROGRAMME, candidate_ids)
        self.assertNotIn(EI_PROGRAMME, candidate_ids)
        self.assertEqual("SEALED_CANDIDATE_COMMITMENTS_UNAPPROVED", self.candidates["status"])
        self.assertEqual("NONE", self.candidates["authority_effect"])

    def test_post_snapshot_admissions_are_additive_and_bounded(self) -> None:
        self.assertEqual("ACTIVE_APPEND_ONLY", self.admissions["status"])
        self.assertEqual(2, self.admissions["admission_count"])
        self.assertEqual(2, len(self.admissions["admissions"]))
        first, second = self.admissions["admissions"]
        self.assertEqual(MG_PROGRAMME, first["programme_id"])
        self.assertEqual("PGN-POST-SNAPSHOT-MG-001", first["admission_id"])
        self.assertEqual("OPERATOR_ADMITTED_BOUNDED_PROGRAMME", first["admission_status"])
        self.assertEqual("POST_SNAPSHOT_OUTSIDE_DEFERRED_PGN_NATIVE_MIGRATION", first["admission_mode"])
        self.assertEqual("NONE", first["frozen_snapshot_effect"])
        self.assertEqual("NONE", first["sealed_sixteen_candidate_population_effect"])
        self.assertEqual("INACTIVE_NONCANONICAL_SHADOW_EXPERIMENT_IMPLEMENTATION_ONLY", first["authority"]["granted"])
        self.assertEqual("NONE", first["authority"]["selector_activation"])
        self.assertEqual("NONE", first["authority"]["rule_promotion"])
        self.assertEqual("NONE", first["authority"]["publication_validation_probability_risk_exposure_execution"])

        self.assertEqual(EI_PROGRAMME, second["programme_id"])
        self.assertEqual("PGN-POST-SNAPSHOT-MG-EI-JUNE-002", second["admission_id"])
        self.assertEqual("OPERATOR_ADMITTED_BOUNDED_PROGRAMME", second["admission_status"])
        self.assertEqual("POST_SNAPSHOT_OUTSIDE_DEFERRED_PGN_NATIVE_MIGRATION", second["admission_mode"])
        ...
        self.assertEqual("BOUNDED_EMPIRICAL_INTEGRATION_ACCEPTED_JUNE_EVIDENCE_SHADOW_EXPERIMENT_ONLY", second["authority"]["granted"])
        ...
        self.assertEqual("NONE", second["authority"]["rule_candidate_semantic_promotion"])
        self.assertEqual("NONE", second["authority"]["publication_active_authority_probability_risk_exposure_execution"])
    ...
    def test_original_operator_decision_qa_state_and_receipt_remain_exact(self) -> None:
        self.assertEqual(MG_PROGRAMME, self.decision["programme_id"])
        self.assertEqual("PASS_BOUNDED_POST_SNAPSHOT_ADMISSION", self.decision["decision"])
        self.assertEqual("INACTIVE_NONCANONICAL_SHADOW_EXPERIMENT_IMPLEMENTATION_ONLY", self.decision["authority_delta"])
        ...
        self.assertEqual("SATISFIED_OPERATOR_DECISION", self.state["authority_required"])
        self.assertEqual("MG-G0-REBASE_AND_RERUN", self.state["next_packet"])
        self.assertEqual("448f8bfa85f4ddc9a7db6cfa8ae3f3aece1c1375", self.state["merge_commit"])
        self.assertTrue(self.receipt["effective"])
        self.assertEqual(339, self.receipt["pull_request"])
        self.assertEqual("e9a299b776f90801588bccf665d187d8c29f3382", self.receipt["final_head"])
        self.assertEqual("448f8bfa85f4ddc9a7db6cfa8ae3f3aece1c1375", self.receipt["merge_commit"])
    ...
    def test_compact_summary_is_printable_for_exact_head_assurance(self) -> None:
        summary = {
            ...
            "admitted_programmes": [item["programme_id"] for item in self.admissions["admissions"]],
        }
        print("PGN_FROZEN_SNAPSHOT_AND_ADMISSION_SUMMARY=" + json.dumps(summary, sort_keys=True, separators=(",", ":")))
