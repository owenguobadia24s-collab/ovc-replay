...
RECEIPT = BASE / "EI_SRFD_SUPERSESSION_POST_MERGE_RECEIPT.json"
...
class EiSrfdSupersessionTests(unittest.TestCase):
    def test_operator_decision_preserves_history_and_supersedes_route(self) -> None:
        ...
        self.assertEqual("OVC APPROVE EI-WP2/EI-WP3 SUPERSEDE_WITH_SRFD_SRI_REPRESENTATIONPACK", decision["operator_command"])
    ...
    def test_crosswalk_points_to_implemented_frozen_srfd_route(self) -> None:
        ...
        self.assertEqual("COMPLETED", crosswalk["status"])
        ...
        freeze = load(SRFD_FREEZE); registry = load(SRFD_REGISTRY)
        self.assertEqual("PREREGISTRATION_FREEZE", freeze["decision"])
        self.assertEqual("7d93994836bfcff6c5a0b39db33692f70b1a25782bee43c7b6329d17568561c0", freeze["approved_candidate"]["representation_pack_registry_v0_2_logical_sha256"])
    ...
    def test_programme_is_superseded_without_rewriting_completed_evidence(self) -> None:
        state = load(STATE); packets = {item["packet_id"]: item for item in state["packets"]}
        self.assertEqual("SUPERSEDED", state["status"])
        self.assertEqual("7857f50a56dbb0527c4284fffa0a12e063730f10", state["merge_commit"])
        ...
        self.assertEqual("CLOSED_UNMERGED", packets["EI-WP3"]["blocked_pr_state"])
        self.assertEqual("SUPERSEDED", packets["EI-WP4"]["status"])
        self.assertIsNone(state["next_packet"])

    def test_final_qa_and_post_merge_receipt(self) -> None:
        qa = load(QA); receipt = load(RECEIPT)
        self.assertEqual("PASS", qa["status"]); self.assertEqual([], qa["blockers"]); self.assertEqual("PASS", qa["qa_recommendation"])
        self.assertEqual("COMPLETED", receipt["status"])
        self.assertEqual("17b687c22d4104c92e2203fd5c389a5b40288b81", receipt["final_head"])
        self.assertEqual("7857f50a56dbb0527c4284fffa0a12e063730f10", receipt["merge_commit"])
        self.assertEqual("CLOSED_SUPERSEDED_HISTORICAL_EVIDENCE", receipt["pr371"]["state"])
        self.assertFalse(receipt["pr371"]["merged"])
        self.assertEqual(0, receipt["tests"]["unresolved_review_threads"])
        self.assertEqual("UNCHANGED_SEPARATELY_AUTHORIZED", receipt["authority_after_merge"]["srfd_june_authority"])
