...
class MarketGrammarWp10GatePacketTests(unittest.TestCase):
 ...
 def test_next_programme_is_bounded_and_separate(self):
  decision=load(DECISION); nxt=decision['next_programme']; self.assertEqual('OVC-MARKET-GRAMMAR-EMPIRICAL-INTEGRATION-JUNE-v0.1',nxt['programme_id']); self.assertIn('accepted June evidence',nxt['scope']); self.assertIn('canonical selection',nxt['stop_before']); self.assertIn('publication',nxt['stop_before'])
