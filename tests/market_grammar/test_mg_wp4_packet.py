...
class MarketGrammarWp4PacketTests(unittest.TestCase):
 ...
 def test_state_preserves_upstream_and_routes_only_wp4(self):
  state=json_at('624b4bfb9b51b1e460fb2cec30f593651569d9ff',STATE); p={x['packet_id']:x for x in state['packets']}; self.assertEqual('COMPLETED',p['MG-WP3']['status']); self.assertIn(p['MG-WP4']['status'],{'RUNNING','IMPLEMENTED','QA_REVIEW','APPROVED','COMPLETED'}); self.assertEqual('OPERATOR_REQUIRED',p['MG-WP10']['authority_required']); self.assertNotIn(state['status'],{'BLOCKED','QUARANTINED'})
