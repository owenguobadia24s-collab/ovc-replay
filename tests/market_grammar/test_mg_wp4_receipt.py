...
class MarketGrammarWp4ReceiptTests(unittest.TestCase):
 ...
 def test_state_unlocks_wp5_without_reserved_delta(self):
  s=json_at('2845a0e4dd2614165965db6ddd81d6955556e0aa',STATE); p={x['packet_id']:x for x in s['packets']}; self.assertEqual('COMPLETED',p['MG-WP4']['status']); self.assertIn(p['MG-WP5']['status'],{'READY','RUNNING','IMPLEMENTED','QA_REVIEW','APPROVED','COMPLETED'}); self.assertEqual('OPERATOR_REQUIRED',p['MG-WP10']['authority_required']); self.assertNotEqual('MG-WP4',s['next_packet'])
