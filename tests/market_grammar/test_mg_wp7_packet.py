...
class MarketGrammarWp7PacketTests(unittest.TestCase):
    def test_packet_preserves_inactive_noncanonical_boundary(self):
        reg=load(REG); ledger=load(LEDGER); features=load(FEATURES); qa=load(BASE/"MG_WP7_QA_PACKET.json"); manifest=load(BASE/"MG_WP7_IMPLEMENTATION_MANIFEST.json")
        self.assertEqual(14,reg["candidate_count"])
        self.assertEqual(14,ledger["candidate_count"])
        self.assertEqual({"MAPPED":14},ledger["migration_status_counts"])
        self.assertFalse(ledger["canonical"])
        self.assertEqual("NONE",ledger["promotion_authority"]); self.assertEqual(ledger["feature_registry_sha256"],features["registry_sha256"]); self.assertFalse(features["canonical"])
        self.assertFalse(reg["candidate_promotion"])
        self.assertFalse(reg["canonical_grammar"])
        self.assertEqual("PASS_ZERO",qa["checks"]["reserved_authority"])
        self.assertEqual([],qa["blockers"])
        self.assertFalse(manifest["candidate_promotion"])

    def test_migration_schemas_are_closed(self):
    ...
    def test_state_preserves_upstream_and_routes_only_wp7(self):
        state=load(STATE); packets={x["packet_id"]:x for x in state["packets"]}
        self.assertEqual("COMPLETED",packets["MG-WP6"]["status"])
        self.assertIn(packets["MG-WP7"]["status"],{"RUNNING","IMPLEMENTED","QA_REVIEW","APPROVED","COMPLETED"})
        self.assertEqual("PLANNED",packets["MG-WP8"]["status"])
