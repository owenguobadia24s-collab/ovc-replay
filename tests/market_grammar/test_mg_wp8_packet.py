...
class MarketGrammarWp8PacketTests(unittest.TestCase):
 ...
 def test_state_completes_wp8_and_unlocks_only_wp9(self):
  state=load(STATE); packets={x['packet_id']:x for x in state['packets']}; self.assertEqual('COMPLETED',packets['MG-WP8']['status']); self.assertEqual('SATISFIED_DELEGATED_DECISION',packets['MG-WP8']['authority_required']); self.assertEqual('READY',packets['MG-WP9']['status']); self.assertEqual('OPERATOR_REQUIRED',packets['MG-WP10']['authority_required']); self.assertEqual('MG-WP9',state['next_packet']); self.assertNotIn(state['status'],{'BLOCKED','QUARANTINED'})
