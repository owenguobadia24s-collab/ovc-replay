...
class C2E2WP7ActivationReadinessTests(unittest.TestCase):
    ...
    def test_operator_continuation_is_exact_and_replay_deferred(self):
        self.assertEqual(
            self.continuation["operator_command"],
            "OVC CONTINUE C2E2-WP7 REPLAY_STATUS=DEFERRED CONTINUE_THROUGH_AUTO_GATES=YES STOP_AT=C2E-AG0",
        )
    ...
    def test_ag0_is_ready_only_for_operator_review_and_later_gates_remain_not_ready(self):
        self.assertEqual(self.packet["replay_status"], "DEFERRED")
    ...
    def test_conflict_denominators_and_replay_gap_are_visible(self):
    ...
    def test_no_active_selector_pack_publication_or_validation_delta(self):
        ...
        after = self.packet["authority_after"]
        self.assertEqual(after["active_c2e"], "NONE")
        self.assertEqual(after["active_boundary_pack"], "NONE")
        self.assertEqual(after["selector_publication_validation"], "DENIED")

    def test_schema_and_qa_are_present_and_gate_is_auto_only_for_completeness(self):
        ...
        self.assertEqual(self.qa["gate_id"], "C2E2-G7")
        self.assertIn(self.qa["status"], {"PASS_PENDING_FINAL_HEAD_ASSURANCE", "PASS"})
        self.assertIn(self.qa["recommendation"], {"PASS_IF_FINAL_HEAD_ASSURANCE", "PASS"})
