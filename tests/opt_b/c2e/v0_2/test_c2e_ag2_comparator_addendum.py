...
def test_classifies_common_transition_surface_and_excludes_stream_start():
    frames = [
    ...
    assert null_control_counts(result["rows"]) == {
        "BOTH_BOUNDARY": 0,
        "C2E_ONLY": 2,
        "SRFD_ONLY": 0,
        "NEITHER_BOUNDARY": 0,
...
def test_family_or_outcome_fields_fail_closed():
    frames = [
        _frame("2026-06-01T00:15:00Z", "o1", None, "s1"),
        _frame("2026-06-01T00:30:00Z", "o2", "o1", "s2"),
    ]
    ...
    with pytest.raises(AG2ComparatorError, match="AG2_FORBIDDEN_COMPARATOR_FIELD"):
