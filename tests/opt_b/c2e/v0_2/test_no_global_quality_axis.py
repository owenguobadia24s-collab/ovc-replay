...
class C2E2NoGlobalQualityAxisTests(unittest.TestCase):
    def test_quality_is_orthogonal_not_structural(self) -> None:
        ...
        }.issubset({key for key in frame["structural"] if key.endswith("_record_ids")}))
        self.assertNotIn("QUALITY", frame["structural"])
        self.assertEqual(frame["diagnostic_namespace"]["quality_evidence"]["status"], "ASSURED")

    def test_missing_optional_parent_does_not_globally_invalidate_local_frame(self) -> None:
        payload = json.loads(FIXTURE.read_text())
        ...
        self.assertEqual(frame["evidence"]["technical_status"], "COMPUTABLE")
        self.assertIsNone(frame["context"]["context_resolution_bundle_id"])
