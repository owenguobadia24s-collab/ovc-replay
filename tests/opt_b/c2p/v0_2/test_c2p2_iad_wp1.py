...
CONTRACT = ROOT / "contracts/research/c2p2_iad/C2P2_IAD_IDENTITY_ANCHOR_EVIDENCE_CONTRACT_v0_1.md"
SCHEMA = ROOT / "schemas/research/c2p2_iad/c2p2_iad_identity_anchor_evidence_v0_1.schema.json"
...
def test_iad_research_contract_does_not_mutate_frozen_c2p_v0_2_catalogue() -> None:
    core_contracts = list((ROOT / "contracts/opt_b/c2p/v0_2").glob("*.md"))
    core_schemas = list((ROOT / "schemas/opt_b/c2p/v0_2").glob("*.json"))
    assert len(core_contracts) == 15
    assert len(core_schemas) == 15
