...
def test_all_semantic_artifact_hashes_and_successor_candidate_hashes_are_exact() -> None:
    ...
    module = ROOT / binding["semantic_components"]["executable_semantics"]["path"]
    assert sha256(module.read_bytes()).hexdigest() == binding["semantic_components"]["executable_semantics"]["sha256"]
    for candidate in candidates["candidates"]:
        assert candidate["candidate_logical_hash"] == _candidate_hash(candidate)
        assert candidate["runtime_binding"]["status"] == "PENDING_EMPIRICAL_RUNTIME_CLOSEOUT"
        assert candidate["runtime_binding"]["implementation_sha256"] is None
        assert candidate["real_source_execution_eligible"] is False

    assert candidates["grun_currentness"]["fresh_grun_required"] is True
    assert candidates["grun_currentness"]["runtime_closeout_required_before_fresh_grun"] is True
...
def test_c2e_dependency_registry_has_zero_current_roles_and_any_future_role_fails_closed() -> None:
    registry = _read(REG / "C2P2_RS0_C2E_DEPENDENCY_ROLE_REGISTRY_v0_1.json")
    ...
    for execution_mode in ("PENDING", "EXACT_BOUND"):
        future = dict(registry)
        future["entries"] = [{"structural_role_id": "EPISODE_RELATIVE_LEVEL", "execution_mode": execution_mode}]
...
@pytest.mark.parametrize("field", ["outcome", "OPT_C", "Validation", "C3_semantics", "risk", "execution"])
def test_forbidden_future_outcome_or_authority_inputs_fail_closed_recursively(field: str) -> None:
    row = _level(record_id="L1", value=1.25)
    row["geometry_signature"]["nested"] = {field: "FORBIDDEN"}
    with pytest.raises(RS0SemanticError, match=rf"RS0_FORBIDDEN_IDENTITY_FIELD:.*{field}"):
