...
def test_base_vertical_reference_path_is_deterministic_and_source_immutable():
    ...
    text1, trace1 = render_reference_statement(record, proposition, ast)
    text2, trace2 = render_reference_statement(record, proposition, ast)
...
def test_family_organisation_constraint_are_optional_and_do_not_invalidate_base_statement():
    ...
    text, trace = render_reference_statement(record, proposition, ast)
    assert text.startswith("Occurrence ")
    assert "OPTIONAL_FAMILY_CONTEXT_NOT_REQUIRED" in trace.omission_reasons
...
def test_shadow_and_production_maturity_fail_closed_without_activation_record():
    for maturity in (C3BridgeMaturity.SHADOW_EVALUATION, C3BridgeMaturity.PRODUCTION_GRAMMAR):
        with pytest.raises(C3BridgeError, match="C3_BRIDGE_ACTIVATION_RECORD_MISSING"):
            require_reference_maturity(maturity)
    with pytest.raises(C3BridgeError, match="C3_BRIDGE_ACTIVATION_FORBIDDEN_IN_ESLI"):
        require_reference_maturity(C3BridgeMaturity.INACTIVE_REFERENCE, activation_state="ACTIVE")


def test_activation_registry_is_empty_and_maturity_registry_is_reference_only():
    activation = json.loads((ROOT / "registries/opt_b/esl/C3_VOCABULARY_ACTIVATION_v0_1.json").read_text(encoding="utf-8"))
    maturity = json.loads((ROOT / "registries/opt_b/esl/C3_BRIDGE_MATURITY_v0_1.json").read_text(encoding="utf-8"))
    assert activation["activation_state"] == "NONE" and activation["bindings"] == []
    assert maturity["current_maturity"] == "INACTIVE_REFERENCE"
...
def test_llm_explanation_is_separate_noncanonical_sidecar():
    ...
    proposition = proposition_from_occurrence(record)
    ast = statement_ast_from_occurrence(record, proposition)
    explanation = C3ExplanationRecord("EXPL.1", ast.statement_id, {"generator":"fixture"}, "Non-authoritative explanation.", (record.occurrence_record_id,), ("NONCANONICAL",))
    assert explanation.noncanonical is True
    assert "generated_text" not in ast.__dict__
    assert "explanation" not in repr(ast).lower()
