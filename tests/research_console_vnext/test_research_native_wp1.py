...
class ResearchNativeWP1Tests(unittest.TestCase):
    def test_census_covers_all_required_source_families(self):
        ...
        ids = {x["capability_id"] for x in value["sources"]}
        self.assertEqual(ids, {"opt_a","c1","c2","c2e","c2p","c2_5","c3","sri_fdi","srfd","mcarb","dmrp_research_operations","evidence","programme_governance","irof"})
        self.assertTrue(all(x["real_route"].startswith("DENIED") for x in value["sources"]))
