...
class ResearchNativeWP4APreparationTests(unittest.TestCase):
    def test_fixture_composer_preserves_c1_c2_and_fails_honest_for_absent_upper_layers(self):
        ...
        self.assertEqual("DENIED_PENDING_RCN_RN_G4", snapshot["real_source_presentation"])
        self.assertEqual(structure["c1"], snapshot["translation"]["c1"])
        self.assertEqual(structure["c2"], snapshot["structure"]["c2"])
        self.assertEqual("NOT_MATERIALIZED", snapshot["structure"]["c2e"]["availability"])
        self.assertEqual("PROHIBITED", snapshot["structure"]["c2e"]["reconstruction"])
        self.assertEqual([], snapshot["structure"]["c2e"]["episodes"])
        self.assertEqual([], snapshot["structure"]["c2e"]["events"])
        self.assertEqual("NOT_MATERIALIZED", snapshot["structure"]["transitions"]["availability"])
        self.assertEqual("PROHIBITED", snapshot["structure"]["transitions"]["synthesis"])
        self.assertEqual([], snapshot["structure"]["transitions"]["items"])
        self.assertEqual(len(market["bars"]), snapshot["market_context"]["bar_count"])
        self.assertEqual("OPTIONAL_CONTEXT_ONLY", snapshot["market_context"]["scientific_role"])
        self.assertNotIn("overall_state", snapshot["structure"]["c2"])
        self.assertNotIn("winner_axis", snapshot["structure"]["c2"])
        self.assertNotIn("confidence_score", snapshot["structure"]["c2"])
    ...
    def test_binding_candidates_are_declarations_not_real_source_bindings(self):
        ...
        self.assertTrue(registry["candidates"])
        for candidate in registry["candidates"]:
            self.assertEqual("PREPARED_NOT_BOUND", candidate["activation_state"])
            self.assertFalse(candidate["real_source_presented"])
            self.assertEqual("NONE", candidate["authority_effect"])
            self.assertEqual("RCN-RN-G4", candidate["gate_required"])

    def test_route_and_programme_state_preserve_g4_boundary(self):
        routes = load(ROUTES)
        self.assertEqual("GET_ONLY", routes["transport"])
        self.assertIn("/investigate/snapshot", routes["domains"]["INVESTIGATE"])
        self.assertEqual("DENIED_UNTIL_RCN_RN_G4", routes["real_source_exposure"])
        self.assertEqual("PREPARED_NOT_BOUND", routes["wp4a_preparation"]["binding_state"])

        state = load(STATE)
        self.assertEqual("RCN-RN-WP4A", state["packet_id"])
        self.assertIn(state["status"], {"RUNNING", "QA_REVIEW", "COMPLETED"})
        ...
        self.assertEqual("RCN-RN-G4_BEFORE_FIRST_REAL_SOURCE_PRESENTATION", state["stop_boundary"])
    ...
    def test_runtime_route_is_fixture_only_and_validation_denies_before_resource_reads(self):
        ...
        response = client.get("/api/v1/investigate/snapshot")
        ...
        self.assertEqual("NONE", body["fixture_banner"]["authority_effect"])
        self.assertEqual("ovc-rcn-investigate-snapshot/v1", body["schema_id"])
        self.assertEqual("FIXTURE_ONLY_G4_PREPARATION", body["payload"]["mode"])
        self.assertEqual("DENIED_PENDING_RCN_RN_G4", body["payload"]["real_source_presentation"])
        self.assertEqual("NOT_MATERIALIZED", body["payload"]["structure"]["c2e"]["availability"])
        self.assertEqual("PROHIBITED", body["payload"]["structure"]["c2e"]["reconstruction"])
        self.assertTrue(all(row["activation_state"] == "PREPARED_NOT_BOUND" for row in body["payload"]["binding_candidates"]))
