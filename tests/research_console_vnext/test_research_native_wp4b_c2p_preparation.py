...
class WP4BC2PPreparation(unittest.TestCase):
 def test_census_and_fixture_distinguish_synthetic_subsystem_from_runtime_owner(self):
  f=load(FIX); c=next(x for x in load(INV)['sources'] if x['capability_id']=='c2p'); upstream=load(C2P_STATE); self.assertTrue(c['repository_materialized']); self.assertTrue(c['synthetic_fixture_subsystem_materialized']); self.assertFalse(c['runtime_owner_materialized']); self.assertEqual('NONE',c['active_source_authority']); self.assertEqual('DENIED',c['real_route']); self.assertEqual('NONE',upstream['authority']['c2p_runtime']); self.assertEqual('DENIED_FUTURE_C2P2_RS0',upstream['authority']['real_source_replay']); self.assertEqual([],f['objects'])
 def test_route_and_current_state_keep_c2p_excluded_after_g4_binding(self):
  r=load(ROUTES); s=load(STATE); self.assertEqual('GET_ONLY',r['transport']); self.assertIn('/c2p/objects',r['domains']['INVESTIGATE']); self.assertFalse(r['wp4b_preparation']['runtime_owner_materialized']); self.assertTrue(r['wp4b_preparation']['synthetic_fixture_subsystem_materialized']); self.assertNotIn('C2P',r['post_g4_binding']['capabilities']); self.assertEqual('RCN-RN-POST-G4-SOURCE-BINDING',s['packet_id']); self.assertEqual('IMPLEMENTED',s['status']); self.assertEqual('AUTO_EXECUTABLE_WITHIN_G4_PASS',s['authority_required']); self.assertEqual('NONE',s['authority_delta']); self.assertIn('OTHERS_DENIED',s['real_source_routes']); self.assertIn('G4_APPROVED_READ_ONLY',s['current_authority'])
