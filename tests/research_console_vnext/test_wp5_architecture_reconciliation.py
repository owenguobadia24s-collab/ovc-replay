...
class TestWp5ArchitectureReconciliation(unittest.TestCase):
    def test_source_faithful_ppm_remains_immutable(self) -> None:
        ...
        governing = (SOURCE / "GOVERNING_SCOPE.md").read_bytes()
        self.assertEqual(hashlib.sha256(governing).hexdigest(), source_ref["source_sha256"])
    ...
    def test_reconciled_packet_split_preserves_parent_lineage(self) -> None:
        ...
        for packet_id in ("WP5B1", "WP5B2", "WP5C1", "WP5C2"):
            manifest = load(RECON / f"PACKET_MANIFEST_RCN_RN_{packet_id}.json")
            self.assertEqual(manifest["authority_delta"], "NONE")

    def test_source_admission_is_explicitly_non_transitive(self) -> None:
        ...
        self.assertIn("G4_MARKET_C1_C2_C2E_AUTHORITY_TRANSITS_TO_RESEARCH_SOURCES", matrix["forbidden_inferences"])
        self.assertEqual(set(matrix["sources"]), {"DMRP", "RCCR", "PRSC_EC1", "OPT_C", "OPT_D"})

    def test_owner_sources_exist_and_console_does_not_own_cross_mode_logic(self) -> None:
        expected = [
            ROOT / "src/ovc/research_operations/dmrp.py",
            ROOT / "src/ovc/research_operations/rccr/post_pilot_read_models.py",
            ROOT / "contracts/research_operations/ec1/prsc/PRSC_REPRESENTATION_TEMPORAL_CONTEXT_CONTRACT_v0_1.md",
            ROOT / "contracts/research_operations/ec1/prsc/PRSC_MULTIPLICITY_CONTRACT_v0_1.md",
            ROOT / "registries/system_atlas/ATLAS_QUERY_POLICY_REGISTRY_v0_1.json",
        ]
        for path in expected:
            self.assertTrue(path.exists(), str(path))
        graph = load(RECON / "RCN_RN_WP5_RECONCILED_PACKET_GRAPH_v0_1.json")
        rccr = next(node for node in graph["nodes"] if node["packet_id"] == "RCN-RN-WP5B2")
        self.assertEqual(rccr["console_role"], "PROJECT_OWNER_READ_MODELS_DO_NOT_REIMPLEMENT")
        self.assertFalse(graph["atlas_integration"]["correctness_dependency"])

    def test_programme_state_advances_only_to_wp5b1(self) -> None:
        state = load(STATE)
        ...
        self.assertEqual(state["current_authority"], "G4_APPROVED_READ_ONLY_REAL_SOURCE_INVESTIGATE_PRESENTATION_MARKET_C1_C2_C2E")
