...
class CCRReviewTests(unittest.TestCase):
 ...
 def test_gate_is_operator_required(self):
  gate=json.loads((ROOT/"docs/releases/clock-continuity-review-v0-1/ccr-g5/CCR_G5_OPERATOR_GATE_PACKET.json").read_text()); self.assertTrue(gate["operator_decision_required"]); self.assertEqual(gate["status"],"GATE_READY"); self.assertEqual(gate["authority_boundary"]["continuity_change_or_activation"],"DENIED")
