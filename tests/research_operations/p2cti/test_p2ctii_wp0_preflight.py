...
QA = ROOT / "docs/programmes/p2cti-v0-1/wp0/P2CTII_WP0_QA_PACKET_v0_1.json"
DECISION = ROOT / "docs/programmes/p2cti-v0-1/wp0/P2CTII_WP0_DELEGATED_DECISION_v0_1.json"
...
def test_owner_authority_is_non_transitive():
    ...
    assert auth["main_advance_reconciliation"]["p2cti_materiality"] == "NONE"
...
def test_wp0_packet_and_delegated_closeout_preserve_authority():
    packet = load(PACKET)
    qa = load(QA)
    ...
    assert qa["recommendation"] == "PASS_AND_INTEGRATE_AFTER_FINAL_HEAD_REQUIRED_CHECKS"
