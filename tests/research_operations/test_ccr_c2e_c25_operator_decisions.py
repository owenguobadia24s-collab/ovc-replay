...
class OperatorDispositionTests(unittest.TestCase):
 ...
 def test_ccr_pass_is_plan_preparation_only(self):
  ...
  self.assertEqual(decision["activation"],"DENIED")
  ...
  self.assertEqual(state["next_packet"],"PCCR-00_PLAN_PREPARATION")
 def test_c2e_is_blocked_before_wp1(self):
  ...
  self.assertEqual(state["packets"][1]["status"],"BLOCKED")
 ...
 def test_multipart_decision_has_no_activation(self):
  ...
  self.assertEqual(value["decisions"]["CCR-G5"]["decision"],"PASS")
  self.assertEqual(value["decisions"]["C2E-G1"]["decision"],"BLOCK")
  self.assertEqual(value["decisions"]["C25-G1"]["decision"],"BLOCK")
  self.assertIn("CLOCK_OR_CONTINUITY_ACTIVATION",value["shared_denials"])
