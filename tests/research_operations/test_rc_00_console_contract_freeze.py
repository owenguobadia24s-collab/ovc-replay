...
class RC00ResearchConsoleContractFreezeTests(unittest.TestCase):
    ...
    def test_preflight_packet_pins_main_and_defers_gate_until_tests(self) -> None:
        packet = json.loads(
            (ROOT / "docs/releases/research-console-v0-2/rc-00/RC_00_PREFLIGHT_PACKET.json").read_text()
        )
        self.assertEqual(packet["baseline"]["commit"], "e365eddb2ee465312ed6563cbe6df0760661f0d7")
    ...
    def test_route_registry_freezes_all_planned_routes_read_only(self) -> None:
    ...
    def test_action_registry_prohibits_mutation_and_deployment(self) -> None:
        ...
        for action_id in (
            "CHANGE_SELECTOR", "CHANGE_THRESHOLD", "ACTIVATE_RELEASE", "EDIT_REPOSITORY",
            "DEPLOY_CONSOLE", "CREATE_MARKET_CLASSIFICATION", "CREATE_PROBABILITY_OR_EXPOSURE_OBJECT",
        ):
    ...
    def test_existing_research_operations_authority_remains_bounded(self) -> None:
