...
class RCG4OperatorGateTests(unittest.TestCase):
    ...
    def test_exact_ro3_g4_merge_and_operator_gate_identity(self) -> None:
        ...
        self.assertEqual(self.packet["status"], "GATE_READY")
    ...
    def test_live_route_is_still_disabled_before_operator_pass(self) -> None:
        current = self.packet["route_and_capability_delta"]["current"]
        proposed = self.packet["route_and_capability_delta"]["proposed_after_pass"]
        self.assertFalse(current["route_enabled"])
        self.assertEqual(current["route_state"], "DISABLED_PENDING_RC_G4")
        self.assertTrue(proposed["route_enabled"])
        self.assertEqual(proposed["live_consumption_authority"], "LOCAL_READ_ONLY_C1_PRESENTATION")
        ...
        self.assertIn("live_console_c1_route: DISABLED_PENDING_RC_G4", self.registry_text)
    ...
    def test_programme_state_stops_at_operator_gate(self) -> None:
        ...
        self.assertEqual(rcg4["status"], "GATE_READY")
        self.assertEqual(rcg4["authority_required"], "OPERATOR_REQUIRED_NOT_AUTO_RATIFIABLE")
        self.assertTrue(rcg4["operator_approval_required"])
        self.assertEqual(rcg4["live_route_current_state"], "DISABLED_PENDING_RC_G4")
