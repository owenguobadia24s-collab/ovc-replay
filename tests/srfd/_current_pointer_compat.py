...
V11_G11_STATE = "registries/implementation/srfd/OVC_SRFDI_STATE_v0_53_WP11_G11_GATE_READY.json"
V11_G11_TERMINAL_STATE = "registries/implementation/srfd/OVC_SRFDI_STATE_v0_55_G11_COMPLETED.json"
...
def assert_lawful_v10_pointer(testcase, pointer: dict) -> bool:
    """Accept exact lawful WP10 v1.0/v1.1 lineage through terminal SRFDI-G11 closeout."""
    if pointer.get("authoritative_state") == V11_G11_TERMINAL_STATE:
        testcase.assertEqual("OVC-SRFD-BENCHMARK-v0.1", pointer["programme_id"])
        testcase.assertEqual("COMPLETED", pointer["status"])
        testcase.assertIsNone(pointer["current_gate"])
        testcase.assertIsNone(pointer["active_packet"])
        ...
        testcase.assertIsNone(pointer["next_packet"])
        testcase.assertEqual("PROGRAMME_COMPLETED_NO_AUTOMATIC_SCIENTIFIC_FOLLOW_ON", pointer["next_action"])
        testcase.assertEqual("PROGRAMME_TERMINAL", pointer["stop_at"])
        testcase.assertFalse(pointer["operator_decision_required"])
        testcase.assertFalse(pointer["g11_operator_decision_pending"])
        testcase.assertEqual("PASS", pointer["g11_operator_decision"])
        testcase.assertEqual("ee313d5324d724ef82584ba9fe6452770201a7a1", pointer["g11_merge_commit"])
        testcase.assertTrue(pointer["science_execution_started"])
        testcase.assertTrue(pointer["science_execution_complete"])
        return True

    if pointer.get("authoritative_state") == V11_G11_STATE:
        testcase.assertEqual("OVC-SRFD-BENCHMARK-v0.1", pointer["programme_id"])
        testcase.assertEqual("GATE_READY", pointer["status"])
        ...
        testcase.assertEqual(V11_HARDENING_V2, pointer["v1_1_hardening_rehearsal_sha256"])
        testcase.assertIsNone(pointer["next_packet"])
        testcase.assertEqual("OPERATOR_DECISION_SRFDI_G11", pointer["next_action"])
        testcase.assertEqual("SRFDI-G11_OPERATOR_SCIENTIFIC_DISPOSITION", pointer["stop_at"])
        testcase.assertTrue(pointer["operator_decision_required"])
        testcase.assertTrue(pointer["g11_operator_decision_pending"])
        testcase.assertIsNone(pointer["g11_operator_decision"])
        testcase.assertTrue(pointer["science_execution_started"])
        testcase.assertTrue(pointer["science_execution_complete"])
        return True
