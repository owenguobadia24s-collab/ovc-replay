...
class SRFDIJuneAuthV07RunScopedTests(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.decision=json.loads(DECISION.read_text()); cls.envelope=json.loads(ENVELOPE.read_text()); cls.token=json.loads(TOKEN.read_text()); cls.qa=json.loads(QA.read_text()); cls.source_reverify=json.loads(SOURCE_REVERIFY.read_text()); cls.pointer=json.loads(POINTER.read_text()); cls.state=json.loads(STATE.read_text())
 def test_authority_artifact_hashes_and_token_reconstruct_exactly(self):
  self.assertEqual("729faf0d9e8128a24f4b1696d15d28aa252af8f5c7c5a82d5d1105f4ca75c244",logical_sha256(self.decision)); self.assertEqual("7e3f41fbb0f62c5ca6644b5efcaee976799ee86bc5a5066d926d48b07935b716",logical_sha256(self.envelope)); self.assertEqual(self.token,june_authority_v07.verify_fresh_june_run_scoped_authority(self.decision,self.envelope,self.token,self.source_reverify)); self.assertEqual(june_authority_v07.EXPECTED_TOKEN,self.token["token_id"]); self.assertEqual(june_authority_v07.RUN_BINDING_SHA256,self.token["run_binding_sha256"])
 def test_resilience_code_and_post_merge_binding_are_exact(self):
  data=RESILIENCE_MODULE.read_bytes(); git_blob=sha1(b"blob "+str(len(data)).encode("ascii")+b"\0"+data).hexdigest(); self.assertEqual(june_authority_v07.RESILIENCE_MODULE_BLOB,git_blob); self.assertEqual(june_authority_v07.RESILIENCE_BINDING_SHA256,logical_sha256(june_authority_v07.implementation_binding())); self.assertEqual(june_authority_v07.RUN_BINDING_SHA256,june_authority_v07.build_run_binding().logical_hash); self.assertEqual(june_authority_v07.BASELINE_MAIN,self.decision["resilience_precondition"]["execution_resilience_merge"]); self.assertEqual(489,self.decision["resilience_precondition"]["exact_head_assurance"]["pr_number"])
 def test_frozen_source_verification_is_reused_without_provider_fetch(self):
  self.assertEqual(june_authority_v07.SOURCE_REVERIFY_HASH,logical_sha256(self.source_reverify)); self.assertTrue(self.source_reverify["all_exact"]); self.assertEqual(6,len(self.source_reverify["artifacts"])); self.assertEqual("REUSE_IMMUTABLE_ACCEPTED_VERIFICATION_NO_PROVIDER_FETCH",self.decision["prerequisites"]["source_artifact_reverification_mode"]); self.assertEqual("DENIED",self.decision["prerequisites"]["provider_fetch"]); self.assertEqual("FORBIDDEN",self.envelope["source_population_binding"]["provider_fetch"]); self.assertEqual("LOCKED_UNCONSUMED",self.envelope["firewalls"]["validation_2025"])
 def test_fresh_token_starts_only_the_exact_bound_run(self):
  binding=june_authority_v07.build_run_binding()
  with TemporaryDirectory() as td:
   store=RunAuthorityStore(Path(td)); start=store.consume(self.token,binding); self.assertEqual("CONSUMED_FOR_RUN",start.state); self.assertEqual(self.token["token_id"],start.token_id); self.assertEqual(june_authority_v07.RUN_BINDING_SHA256,start.run_binding_sha256); self.assertEqual(start,store.load(self.token["token_id"]))
 ...
 def test_historical_v07_state_is_immutable_while_pointer_may_advance(self):
  ...
  if self.pointer["authority_token_id"]!=self.token["token_id"]:
   self.assertEqual(self.token["token_id"],self.pointer["prior_v0_7_authority_token_id"]); self.assertEqual("SUPERSEDED_UNUSED_UNCONSUMED",self.pointer["prior_v0_7_authority_token_state"]); self.assertFalse(self.pointer["prior_v0_7_authority_token_consumed"])
  else:
