...
class SRFDIWP10V09CapacityFailureTests(unittest.TestCase):
 ...
 def test_v09_is_blocked_not_completed(self):
  self.assertEqual('BLOCKED_PRESERVED_NOT_COMPLETED',self.f['status']); self.assertEqual('BLOCKED',self.s['status']); self.assertEqual('BLOCKED',self.p['status']); self.assertEqual(RUN,self.f['run_id'])
