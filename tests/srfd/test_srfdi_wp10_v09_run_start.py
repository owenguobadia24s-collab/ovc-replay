...
class SRFDIWP10V09RunStartTests(unittest.TestCase):
 @classmethod
 def setUpClass(cls): cls.e=json.loads(E.read_text()); cls.s=json.loads(S.read_text()); cls.f=json.loads(F.read_text())
 ...
 def test_historical_running_state_is_immutable_and_does_not_depend_on_current_pointer(self):
  self.assertEqual('RUNNING',self.s['status']); self.assertEqual(RUN,self.s['run_id']); self.assertEqual(BIND,self.s['run_binding_sha256']); self.assertEqual('SRFDI-G10',self.s['current_gate']); self.assertEqual('SRFDI-WP10-v0.9-RESUME',self.s['next_packet']); self.assertEqual('DENIED',self.s['authority']['provider_fetch']); self.assertEqual('LOCKED_UNCONSUMED',self.s['authority']['validation_2025'])
 def test_later_v09_capacity_failure_remains_separate_historical_evidence(self):
  self.assertEqual('BLOCKED_PRESERVED_NOT_COMPLETED',self.f['status']); self.assertEqual(1626,self.f['work_units']['completed_unit_count']); self.assertEqual(394,self.f['work_units']['remaining_unit_count'])
