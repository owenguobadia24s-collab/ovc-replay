...
class DevelopmentObservabilityTests(unittest.TestCase):
    def test_measured_span_and_unobserved_wall(self) -> None:
        ...
        trace = DevelopmentTrace(
            run_id="RUN-1", programme_id="P", packet_id="WP",
            clock_ns=clock.clock_ns, utc_now=clock.utc_now,
        )
    ...
    def test_overlap_is_union_not_sum(self) -> None:
        ...
        summary = summarize_trace(
            run_id="RUN-2", programme_id=None, packet_id=None,
            started_at_utc="2026-08-13T17:00:00Z", completed_at_utc="2026-08-13T17:00:10Z",
            total_wall_ms=10_000, events=events,
        )
    ...
    def test_subprocess_wrapper_measures_without_authorizing(self) -> None:
        ...
        result = observe_subprocess(
            trace, ["python", "-m", "unittest"],
            category="LOCAL_TEST_ASSURANCE", runner=fake_runner,
        )
    ...
    def test_github_jobs_queue_and_execution_are_ingested(self) -> None:
        ...
        events = ingest_github_workflow_jobs(
            trace,
            workflow_run={"id": 10, "name": "tests"},
            jobs=[{
                "id": 11,
                "name": "repository-suite",
                "created_at": "2026-08-13T17:00:01Z",
                "started_at": "2026-08-13T17:00:03Z",
                "completed_at": "2026-08-13T17:00:08Z",
                "conclusion": "success",
            }],
        )
    ...
    def test_companion_receipt_is_identity_inert(self) -> None:
        summary = summarize_trace(
            run_id="RUN-6", programme_id="P", packet_id="WP",
            started_at_utc="2026-08-13T17:00:00Z", completed_at_utc="2026-08-13T17:00:01Z",
            total_wall_ms=1000, events=[],
        )
        source = {"schema": "existing-orch-record/v1", "record_id": "ORIGINAL-ID", "status": "PASS"}
        receipt = build_companion_receipt(
            source_execution_record=source,
            trace_summary=summary,
            observed_at_utc="2026-08-13T17:00:02Z",
        )
        ...
        self.assertEqual(attached["development_latency_diagnostic"]["receipt_class"], "TEMPORARY_DIAGNOSTIC_OBSERVABILITY")
        self.assertFalse(attached["development_latency_diagnostic"]["governance_expansion"])
        self.assertFalse(attached["development_latency_diagnostic"]["new_operator_gate"])

    def test_policy_is_diagnostic_only(self) -> None:
        root = Path(__file__).resolve().parents[1]
        policy = json.loads(
            (root / "registries/development/skills/development_latency_diagnostic_observability_v0_1.json").read_text()
        )
        ...
        self.assertEqual(
            policy["measurement_contract"]["model_reasoning"],
