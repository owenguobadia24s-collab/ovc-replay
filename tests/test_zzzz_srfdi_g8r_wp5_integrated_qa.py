...
def records(n: int = 14) -> list[dict[str, object]]:
    return [
        {
            "representation_id": f"R{index:04d}",
            "comparability_domain_id": "D0",
            "ordering_semantics": "STATIC",
            "missingness": {},
            "structural_raw": {
                "x": format(index / 100, ".4f"),
                "y": format((index * 7 % 23) / 100, ".4f"),
            },
        }
...
def random_matrix(n: int, seed: int) -> DistanceMatrix:
    ...
    values = {f"{a}|{b}": rng.choice(choices) for a, b in combinations(ids, 2)}
...
class SRFDIG8RWP5IntegratedQATests(unittest.TestCase):
    ...
    def test_exact_coefficient_8_16_and_precision_boundaries(self) -> None:
        for value, places in (("0", 12), ("1.234567890123", 12), ("-0.000000000001", 12)):
            ...
            self.assertEqual(value, coefficient_to_decimal(coefficient, places))
    ...
    def test_cache_identity_corruption_and_restart_no_recompute(self) -> None:
        cache = SemanticCache()
        identity = {"pair_id": "P", "distance_spec_hash": "S", "population_hash": "POP"}
        ...
        ledger.register_complete("T", content_hash="abc", attempt_id="A1")
        self.assertFalse(ledger.should_compute("T", expected_hash="abc"))
        self.assertEqual(("A1",), ledger.attempts("T"))
        self.assertTrue(ledger.should_compute("T", expected_hash="def"))
        self.assertEqual("QA_CACHE_CORRUPTION", ledger.quarantined["T"])
    ...
    def test_scheduler_keeps_complete_scientific_dag_when_capacity_unresolved(self) -> None:
        ...
        contracts: list[dict[str, object]] = [{
            "node_id": "distance", "node_type": "DISTANCE", "dependency_ids": [],
            "wall_seconds": None, "peak_rss_bytes": None, "external_bytes": None,
            "measurement_class": "UNRESOLVED", "reusable": True,
        }]
        for method in methods:
            contracts.append({
                "node_id": f"family.{method}", "node_type": "FAMILY_METHOD",
                "method_id": method, "configuration_id": "C", "dependency_ids": ["distance"],
                "wall_seconds": "1", "peak_rss_bytes": 1, "external_bytes": 1,
                "measurement_class": "MEASURED", "required": True,
            })
        plan = build_capacity_plan(contracts, required_method_configurations=tuple((method, "C") for method in methods), t0=t0, t1=t1)
        self.assertEqual(5, len(plan["execution_order"]))
        self.assertEqual(4, len(plan["blocked_nodes"]))
        self.assertEqual("PROHIBITED", plan["partial_benchmark_escape_hatch"])
        self.assertEqual("NONE", plan["scientific_effect"])
    ...
    def test_logical_identities_do_not_depend_on_temp_paths(self) -> None:
        ...
        self.assertEqual(logical_sha256(payload), logical_sha256(dict(payload)))
        with tempfile.TemporaryDirectory() as left, tempfile.TemporaryDirectory() as right:
            self.assertNotIn(left, logical_sha256(payload))
            self.assertNotIn(right, logical_sha256(payload))
